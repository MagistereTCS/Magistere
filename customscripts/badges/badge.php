...
if (!empty($badge->recipient->id)) {
    ...
    if ($hide) {
        require_sesskey();
        $DB->set_field('badge_issued', 'visible', 0, ['badgeid' => $badge->badgeid, 'userid' => $USER->id]);
        $action = new moodle_url('/badges/badge.php', array('hash' => $id));
        $action = $action->out(false);
        redirect($action);
    } else if ($show) {
        require_sesskey();
        $DB->set_field('badge_issued', 'visible', 1, ['badgeid' => $badge->badgeid, 'userid' => $USER->id]);
        $action = new moodle_url('/badges/badge.php', array('hash' => $id));
        $action = $action->out(false);
        redirect($action);
    }
    ...
    if($delete) {
        $DB->delete_records('badge_issued', ['badgeid' => $badge->badgeid, 'userid' => $USER->id]);
        $DB->delete_records('badge_manual_award', ['badgeid' => $badge->badgeid, 'recipientid' => $USER->id]);
        $action = new moodle_url('/badges/mybadges.php');
        $action = $action->out(false);
        redirect($action, get_string('eventbadgerevoked', 'badges'), null, \core\output\notification::NOTIFY_SUCCESS);
    }
