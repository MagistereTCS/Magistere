    ...
    if (has_capability('moodle/blog:view', $sitecontext)) {
        ...
        if ($CFG->bloglevel >= BLOG_SITE_LEVEL) {
            ...
            if($section){
                $options['courseview'] = array(
                    ...
                    'link' => new moodle_url('/blog/index.php', array('courseid' => $course->id, 'section' => $section))
                );
            } else {
                $options['courseview'] = array(
                    'string' => get_string('viewcourseblogs', 'blog'),
        ...
        if($section){
            $options['courseviewmine'] = array(
                ...
                'link' => new moodle_url('/blog/index.php', array('courseid' => $course->id, 'userid' => $USER->id, 'section' => $section))
            );
        } else {
            $options['courseviewmine'] = array(
                'string' => get_string('viewmyentriesaboutcourse', 'blog'),
    ...
    if (has_capability('moodle/blog:create', $sitecontext)) {
        if($section){
            ...
            $options['courseadd'] = array(
                ...
                'link' => new moodle_url('/blog/edit.php', array('action' => 'add', 'courseid' => $course->id, 'section' => $section))
            );
        } else {
            // We can blog about this course.
            $options['courseadd'] = array(
...
function blog_get_headers($courseid=null, $groupid=null, $userid=null, $tagid=null) {
    ...
    $section = optional_param('courseid', null, PARAM_INT);
    ...
    if (empty($CFG->enableblogs) || !has_capability('moodle/blog:view', $systemcontext)) {
        ...
        if (!empty($blogs)) {
            ...
            foreach ($blogs as $blog) {
                ...
                $url = new moodle_url('/blog/index.php', array('entryid' => $blog->id, 'courseid' => $context->instanceid));
