...
class mod_scorm_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $scormtypes[SCORM_TYPE_CENTRALIZED_RESOURCE] = get_string('centralizedresource', 'scorm');
        ...
        if (count($scormtypes) > 1) {
            ...
            $mform->addElement('text', 'crid', get_string('rcpackageurl', 'scorm'), array('size' => 60, 'readonly' => true));
            $mform->setType('crid', PARAM_RAW);
            $mform->addElement('button', 'cr_selector', get_string('centralizedresource_button', 'scorm'));

            $centralizedrepo = $DB->get_record_sql('
SELECT ri.id
FROM {repository} r			
...
repositories['.$data->id.'] = '.json_encode($data).';
var options = { 
	formcallback: function(data){
		$("#id_crid").val(data.url);
	},
	userprefs: {
	...
	crexternallink: true,
	returntype: 1,
	accepted_types: [".scorm"]
};

$("#id_cr_selector").on("click", function(){
    M.core_filepicker.show(Y, options);
});


var show_hide_field = function(){
    if($("#id_scormtype").val() == "'.SCORM_TYPE_CENTRALIZED_RESOURCE.'"){
        $("#fitem_id_crid").show();
        $("#fitem_id_packageurl").hide();
    }else{
        $("#fitem_id_crid").hide();
        $("#fitem_id_packageurl").show();
    }
};

$("#id_scormtype").on("change", function(){
    if($("#id_scormtype").val() == "'.SCORM_TYPE_CENTRALIZED_RESOURCE.'"){
        $("#fitem_id_crid").show();
        $("#fitem_id_packageurl").hide();
    }else{
...
show_hide_field();
    ...
    public function validation($data, $files) {
        ...
        } else if ($type === SCORM_TYPE_CENTRALIZED_RESOURCE){
            $reference = $data['crid'];

            if (!preg_match('/\[\[\[cr_link_[a-f0-9]{40}\]\]\]/', $reference)) {
                $errors['packageurl'] = get_string('invalid_centralizedresource', 'scorm');
            }
        }
    ...
    public function set_data($defaultvalues) {
        ...
        if (isset($defaultvalues['scormtype']) and isset($defaultvalues['reference'])) {
            switch ($defaultvalues['scormtype']) {
                ...
                case SCORM_TYPE_CENTRALIZED_RESOURCE:
                    $defaultvalues['crid'] = $defaultvalues['reference'];
                    break;
