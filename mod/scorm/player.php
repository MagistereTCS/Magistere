...
if($scorm->scormtype == SCORM_TYPE_CENTRALIZED_RESOURCE){
    $scorm->scormtype = SCORM_TYPE_EXTERNAL;
    $scorm->reference = convert_cr_reference_to_url($scorm->reference);
}
...
$exiturl = course_get_url($course, $cm->sectionnum);
if($displaymode == 'popup'){
    $exitlink = html_writer::link($exiturl, $strexit, array('title' => $strexit, 'class' => 'btn', 'onclick' => "window.open('', '_self', ''); window.close();"));
} else {
