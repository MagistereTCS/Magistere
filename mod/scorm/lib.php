...
define('SCORM_TYPE_CENTRALIZED_RESOURCE', 'centralizedresource');
...
function scorm_add_instance($scorm, $mform=null) {
    ...
    if ($record->scormtype === SCORM_TYPE_LOCAL) {
        ...
        $record->reference = $scorm->packageurl;
    } else if ($record->scormtype === SCORM_TYPE_CENTRALIZED_RESOURCE) {
        $record->reference = $scorm->crid;
...
function scorm_update_instance($scorm, $mform=null) {
    ...
    if ($scorm->scormtype === SCORM_TYPE_LOCAL) {
        ...
        $scorm->reference = $scorm->packageurl;
    } else if ($scorm->scormtype === SCORM_TYPE_CENTRALIZED_RESOURCE) {
        $scorm->reference = $scorm->crid;
...
// TCS START - NNE - 2354
function convert_cr_reference_to_url($reference){
    global $CFG;
    
    if(preg_match('/\[\[\[cr_link_([a-f0-9]{40})\]\]\]/', $reference, $matches)) {
        $dbconn = get_centralized_db_connection();
        $cr_resource = $dbconn->get_record("cr_resources", array('resourceid' => $matches[1]));
        if (isset($cr_resource)) {
            $url = secure_url_scorm('/', $cr_resource->hashname . $cr_resource->createdate, $CFG->secure_link_timestamp_default);
            return $url . 'imsmanifest.xml';
        }
    }
...
function secure_url_scorm($url_base, $filehash, $secure_link_expires){
    global $CFG;
    ...
    $secure_url = $CFG->magistere_domaine.'/md/'.$hash.'/'.$secure_link_expires.'/'.$filehash.$uri;
