...
class report extends \mod_scorm\report {
     ...
     */
    protected function get_data($scoid, $course, $allowedlist = []) {
        global $DB;
        ...
        $sql .= ' AND st.userid IN (SELECT ue.userid FROM {user_enrolments} ue INNER JOIN {enrol} e ON (e.id=ue.enrolid) WHERE e.courseid = '.$course->id.')';
        ...
        if ($scoes = $DB->get_records('scorm_scoes', array("scorm" => $scorm->id), 'sortorder, id')) {
            foreach ($scoes as $sco) {
                if ($sco->launch != '') {
                    ...
                    $data = $this->get_data($sco->id, $course, $allowedlist);
