...
class restore_etherpadlite_activity_structure_step extends restore_activity_structure_step {
    protected function define_structure() {
        ...
        if ($userinfo) {
            ...
            $paths[] = new restore_path_element('etherpadlite_groups', '/activity/etherpadlite/etherpadlite_groups');
        }
    ...
    protected function process_etherpadlite_groups($data) {
        global $DB;
        $data = (object)$data;
        $oldid = $data->id;
        
        $data->etherpadid = $this->get_new_parentid('etherpadlite');
        $data->groupid = $this->get_mappingid('group', $data->groupid);
        $data->timecreated = $this->apply_date_offset($data->timecreated);
        $data->timemodified = $this->apply_date_offset($data->timemodified);
        
        $DB->insert_record('etherpadlite_groups', $data);
    }
