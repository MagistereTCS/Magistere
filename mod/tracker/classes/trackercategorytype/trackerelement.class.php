    ...
    */
    ...
    function optionlistview($cm) {
        ...
        if (!empty($this->options)) {
            foreach ($this->options as $option) {
                $actions  = "<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=editelementoption&amp;optionid={$option->id}&amp;elementid={$option->elementid}\" title=\"".get_string('edit')."\"><img src=\"".$OUTPUT->image_url('/t/edit', 'core')."\" /></a>&nbsp;" ;
                $img = ($option->sortorder > 1) ? 'up' : 'up_shadow' ;
                $actions .= "<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=moveelementoptionup&amp;optionid={$option->id}&amp;elementid={$option->elementid}\" title=\"".get_string('up')."\"><img src=\"".$OUTPUT->image_url("{$img}", 'mod_tracker')."\"></a>&nbsp;";
                $img = ($option->sortorder < $this->maxorder) ? 'down' : 'down_shadow' ;
                $actions .= "<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=moveelementoptiondown&amp;optionid={$option->id}&amp;elementid={$option->elementid}\" title=\"".get_string('down')."\"><img src=\"".$OUTPUT->image_url("{$img}", 'mod_tracker')."\"></a>&nbsp;";

                $actions .= "<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=deleteelementoption&amp;optionid={$option->id}&amp;elementid={$option->elementid}\" title=\"".get_string('delete')."\"><img src=\"".$OUTPUT->image_url('/t/delete', 'core')."\"></a>";
