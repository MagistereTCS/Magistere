...
if (!empty($issues)) {
    ...
    foreach ($issues as $issue) {
        ...
        if (has_capability('mod/tracker:manage', $context) || has_capability('mod/tracker:resolve', $context)) {
            $actions = "<a href=\"view.php?id={$cm->id}&amp;issueid={$issue->id}&screen=editanissue\" title=\"".get_string('update')."\" ><img src=\"".$OUTPUT->image_url('/t/edit')."\" border=\"0\" /></a>";
        }
        if (has_capability('mod/tracker:manage', $context)) {
            $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;issueid={$issue->id}&what=delete\" title=\"".get_string('delete')."\" ><img src=\"".$OUTPUT->image_url('/t/delete')."\" border=\"0\" /></a>";
        }
        // Ergo Report I3 2012 => self list displays owned tickets. Already registered
        // $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;view=profile&amp;screen=mywatches&amp;issueid={$issue->id}&what=register\" title=\"".get_string('register', 'tracker')."\" ><img src=\"".$OUTPUT->image_url('register', 'tracker')."\" border=\"0\" /></a>";
        if (($issue->resolutionpriority < $maxpriority) && has_capability('mod/tracker:viewpriority', $context) && !has_capability('mod/tracker:managepriority', $context)) {
            $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;issueid={$issue->id}&amp;what=askraise\" title=\"".get_string('askraise', 'tracker')."\" ><img src=\"".$OUTPUT->image_url('askraise', 'tracker')."\" border=\"0\" /></a>";
        }
        if (!empty($tracker->parent)) {
