...
if (!empty($issues)) {
    ...
    foreach ($issues as $issue) {
        ...
        $solution = ($hassolution) ? "<img src=\"".$OUTPUT->image_url('solution', 'tracker').'" height="15" alt="'.get_string('hassolution','tracker')."\" />" : '' ;
        $actions = '';
        ...
        if (preg_match('/^resolutionpriority/', $sort) && has_capability('mod/tracker:managepriority', $context)) {
            ...
            if ($issue->resolutionpriority < $maxpriority) {
                ...
                $actions .= "&nbsp;<img src=\"".$OUTPUT->image_url('up_shadow', 'mod_tracker')."\" border=\"0\" />";
                $actions .= "&nbsp;<img src=\"".$OUTPUT->image_url('totop_shadow', 'mod_tracker')."\" border=\"0\" />";
            }
