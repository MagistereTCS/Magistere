    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            if($this->get_instance()->grade === "0"){
                ...
                $cm = $DB->get_record('course_modules',array("module" => 1,"instance"=>$this->get_instance()->id));
                ...
                if($submission && $submission->status =="submitted"){
                    ...
                    foreach ($comments as $comment){
                        if(has_capability("mod/assign:grade",context_course::instance($this->get_instance()->course),$comment->userid)){
                            $commentbyassigngrade = true;
                        }
                    }
                    if($commentbyassigngrade && $this->get_instance()->completionsubmit){
