...
class assign_grading_table extends table_sql implements renderable {
    ...
    public function __construct(assign $assignment,
        ...
        /* ARO 3537 EDIT IF CONFIDTION */
        if (!$this->is_downloading() && $assignment->get_instance()->grade !== "0") {
            // Final grade.
            $columns[] = 'finalgrade';
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            ...
            $link = '<a href="' . $url . '" class="btn btn-primary">' . get_string('tograde',"assign") . '</a>';
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            ...
            if (!$instance->markingworkflow) {
                ...
                if($instance->grade === "0"){
                    ...
                    $cm = $DB->get_record('course_modules',array("module" => 1,"instance"=>$instance->id));
                    $cm_context = $DB->get_record("context", array("contextlevel" =>CONTEXT_MODULE,"instanceid" => $cm->id));
                    $submission = $DB->get_record("assign_submission",array("assignment" => $instance->id , "userid" => $row->id));

                    if($submission && $submission->status =="submitted" ){
                        $comments = $DB->get_records("comments", array("contextid" => $cm_context->id,"component"=>"assignsubmission_comments","commentarea" => "submission_comments","itemid"=>$submission->id));
                        $commentbyassigngrade = false;
                        foreach ($comments as $comment){
                            if(has_capability("mod/assign:grade",context_course::instance($instance->course),$comment->userid)){
                                $commentbyassigngrade = true;
                            }
                        }
                        if($commentbyassigngrade && $instance->completionsubmit){
