...
class mod_folder_renderer extends plugin_renderer_base {
    ...
    public function display_folder(stdClass $folder) {
        ...
        if (trim($folder->intro) && $cm->showdescription) {
        ...
        if($folder->display == FOLDER_DISPLAY_PAGE) {
    ...
    public function aardvark_custom_section_content($cm){
        global $DB;
        ...
        if (trim($folder->intro) && $cm->showdescription) {
            if ($folder->display != FOLDER_DISPLAY_INLINE) {
                ...
                // for "display inline" do not filter, filters run at display time.
                $output .= format_module_intro('folder', $folder, $cm->id, true);
            }
        }
        ...
        if (has_capability('mod/folder:managefiles', $context)) {
            $buttons .= $this->output->container_start('mdl-align');
            $buttons .= $this->output->single_button(new moodle_url('/mod/folder/edit.php', array('id' => $cm->id)), get_string('edit'), 'mdl-align folder-edit-button');
            $buttons .= $this->output->container_end();
        }
