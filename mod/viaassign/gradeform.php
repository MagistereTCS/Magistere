 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_viaassign_grade_form extends moodleform {
    /** @var viaassignment $viaassignment */
    private $viaassignment;
    ...
    public function definition() {
        ...
        list($viaassignment, $data, $params) = $this->_customdata;
        // Visible elements.
        $this->viaassignment = $viaassignment;
        $viaassignment->add_grade_form_elements($mform, $data, $params);
    ...
    public function validation($data, $files) {
        ...
        if ($instance->grade > 0) {
            if (unformat_float($data['grade']) === null && (!empty($data['grade']))) {
                $errors['grade'] = get_string('invalidfloatforgrade', 'viaassign', $data['grade']);
            } else if (unformat_float($data['grade']) > $instance->grade) {
                $errors['grade'] = get_string('gradeabovemaximum', 'viaassign', $instance->grade);
            } else if (unformat_float($data['grade']) < 0) {
                $errors['grade'] = get_string('gradebelowzero', 'viaassign');
            }
        } else {
            // This is a scale.
            if ($scale = $DB->get_record('scale', array('id' => -($instance->grade)))) {
                ...
                if ((int)$data['grade'] !== -1 && !array_key_exists((int)$data['grade'], $scaleoptions)) {
                    $errors['grade'] = get_string('invalidgradeforscale', 'viaassign');
                }
            }
        }
        return $errors;
