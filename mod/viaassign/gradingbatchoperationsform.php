 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_viaassign_grading_batch_operations_form extends moodleform {
    /**
...
//     */
    public function definition() {
        ...
        // Visible elements.
        $options = array();
        $options['lock'] = get_string('locksubmissions', 'viaassign');
        $options['unlock'] = get_string('unlocksubmissions', 'viaassign');
        if ($instance['duedate'] && has_capability('mod/viaassign:grantextension', $instance['context'])) {
            $options['grantextension'] = get_string('grantextension', 'viaassign');
        }
        ...
        $objs = array();
        $objs[] =& $mform->createElement('select', 'operation', get_string('chooseoperation', 'viaassign'), $options);
        $objs[] =& $mform->createElement('submit', 'submit', get_string('go'));
        $batchdescription = get_string('batchoperationsdescription', 'viaassign');
