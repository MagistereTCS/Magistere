...
M.mod_viaassign.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_viaassign.init_grading_table = function(Y) {
        ...
        if (batchform) {
            batchform.on('submit', function(e) {
                ...
                if (selectedusers.length == 0) {
                    alert(M.str.viaassign.nousersselected);
                    e.preventDefault();
                } else {
                    ...
                    if (action.indexOf(prefix) == 0) {
                        ...
                        confirmmessage = eval('M.str.viaassignfeedback_' + plugin + '.batchoperationconfirm' + action);
                    } else {
                        confirmmessage = eval('M.str.viaassign.batchoperationconfirm' + operation.get('value'));
                    }
...
M.mod_viaassign.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_viaassign.init_grade_change = function(Y) {
    ...
    if (gradenode) {
        ...
        gradenode.on('change', function() {
            if (gradenode.get('value') != originalvalue) {
                alert(M.str.mod_viaassign.changegradewarning);
            }
        });
    }
};

M.mod_viaassign.init_plugin_summary = function(Y, subtype, type, submissionid) {
