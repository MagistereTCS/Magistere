...
/**
 * This file contains the classes for the admin settings of the viaassign module.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class viaassign_admin_page_manage_viaassign_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/viaassign/adminmanageplugins.php', array('subtype' => $subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'viaassign'),
                            $url);
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class viaassign_plugin_manager {
    ...
    /**
     * Constructor for this viaassignment plugin manager
     * @param string $subtype - viaassignfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/viaassign/adminmanageplugins.php', array('subtype' => $subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'viaassign'),
                get_string('version'), get_string('hideshow', 'viaassign'),
        ...
        $shortsubtype = substr($this->subtype, strlen('viaassign'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/viaassign/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'viaassign'));
    }
