...
function url_get_coursemodule_info($coursemodule) {
    ...
    $info->name = $url->name;
   	...
   	if(preg_match('/^\[\[\[cr_link_([0-9a-f]{40})\]\]\]/i', $url->externalurl, $matches))
   	{
   		$ressourceid = $matches[1];
   		$result = get_centralized_db_connection()->get_record_sql(
   				"SELECT r.name, r.hashname, r.cleanname, r.createdate, r.extension, r.type
    			FROM cr_resources r
    			WHERE r.resourceid = '".$ressourceid."' AND r.public = 1");
   		
   		$url_resource = '/'.$CFG->centralizedresources_media_types[$result->type].'/'.$result->cleanname;
   		
   		$url = secure_url($url_resource, $result->hashname.$result->createdate, $CFG->secure_link_timestamp_video);
   	}

    //note: there should be a way to differentiate links from normal resources
    $info->icon = url_guess_icon($url->externalurl, 24);
    $info->content = '<span class ="urldetails">lien</span>';
