...
function url_appears_valid_url($url) {
	if(preg_match('/^\[\[\[cr_link_([0-9a-f]{40})\]\]\]/i', $url))
	{
		return true;
	}else if (preg_match('/^(\/|https?:|ftp:)/i', $url)) {
...
function url_fix_submitted_url($url) {
    ...
    if (!preg_match('|^[a-z]+:|i', $url) && !preg_match('|^/|', $url) && !preg_match('/^\[\[\[cr_link_([0-9a-f]{40})\]\]\]/i', $url)) {
...
function url_get_final_display_type($url) {
    ...
    static $download = array('application/zip', 'application/x-tar', 'application/g-zip',     // binary formats
                             'application/pdf');  // these are known to cause trouble for external links, sorry
    static $embed    = array('image/gif', 'image/jpeg', 'image/png', 'image/svg+xml',         // images
                             'application/x-shockwave-flash', 'video/x-flv', 'video/x-ms-wm', // video formats
                             'video/quicktime', 'video/mpeg', 'video/mp4',
                             'audio/mp3', 'audio/x-realaudio-plugin', 'x-realaudio-plugin',   // audio formats,
                            );
    static $sitepage = array('text/html');
    ...
    if (in_array($mimetype, $embed) || in_array($mimetype, $sitepage)) {
        //TCS 2214 In case of videos, images or audio, url is treated like "DisplayOpen" resource.
        return RESOURCELIB_DISPLAY_OPEN;
