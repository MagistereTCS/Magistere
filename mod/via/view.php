...
if (has_capability('mod/via:view', $context) && (is_mobile_phone() == false || $via->isnewvia == 1)
    ...
    if ($via->recordingmode == 1 || (isset($pbsync) && has_capability('mod/via:manage', $context))) { // Si on est en mode unifi�
        $via->playbacksync = 0;
    }
...
//TCS2020 Modification pour M@gistere pour afficher l'encart custom a la place du heading standard
echo $OUTPUT->add_encart_activity('<h2>'.$via->name.'</h2>');
            ...
            switch($access) {
                case 1:
                    ...
                    <label for="checkbox" style="margin-right: 10px;">'.get_string('recordaccept', 'via').'</label>'.
                        ...
                        // Pas de bouton d�sactiv� pour viaassign.
