...
class H5PEditorAjax {
  ...
  private function fileUpload($contentId = NULL) {
    ...
    if ($file->validate()) {
      $file_path = $this->core->fs->saveFile($file, 0);
      //$this->storage->markFileForCleanup($file_id);
    }
    $file->printResult($file_path);
  }
  ...
  private function libraryUpload($uploadFilePath, $contentId) {
    ...
    $content = $this->core->fs->moveContentDirectory($this->core->h5pF->getUploadedH5pFolderPath(), $contentId);

    // Clean up
    $this->storage->removeTemporarilySavedFiles($this->core->h5pF->getUploadedH5pFolderPath());
    // Change the files name to match the centralized files
    if (strpos($content->contentJson, '"path"'))
    {
        $match = preg_match_all('/("path":"([^\"]*?)")/', $content->contentJson,$matches);
        
        if ($match)
        {
            for($i=0;$i<count($matches[0]);$i++)
            {
                $filematch = $file = $matches[2][$i];
                
                $file = substr($file, strpos($file, '/')+1);
                if (strpos($file, '#') !== false) {
                    $file = substr($file, 0, strrpos($file, '#'));
                }
                $cfile = get_centralized_db_connection()->get_record_sql("SELECT id,contenthash FROM `hvp_files` WHERE filename = :path",array('path'=>$file));
                if ($cfile !== false)
                {
                    $extension = substr(strrchr($file,'.'),1);
                    $file = $cfile->contenthash.'.'.$extension;
                    
                    $content->contentJson = str_replace($filematch,$file,$content->contentJson);
                }
            }
        }
    }
