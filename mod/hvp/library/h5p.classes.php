...
class H5PStorage {
  ...
  public function savePackage($content = NULL, $contentMainId = NULL, $skipContent = FALSE, $options = array()) {
    ...
    if (!$skipContent) {
      ...
      if (strpos($content['params'], '"path"'))
      {
          $match = preg_match_all('/("path":"([^\"]*?)")/', $content['params'],$matches);
          ...
          if ($match)
          {
              for($i=0;$i<count($matches[0]);$i++)
              {
                  ...
                  if ($cfile !== false)
                  {
                      ...
                      $content['params'] = str_replace($filematch,$file,$content['params']);
                  }
              }
          }
      }
...
class H5PCore {
  ...
  const DISPLAY_OPTION_DISPLAYMOD = 'displaymod';
  const DISPLAY_OPTION_DISPLAYMOD_DEFAULT = 0;
  const DISPLAY_OPTION_DISPLAYMOD_INTEGRATED = 1;
