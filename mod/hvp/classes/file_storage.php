...
class file_storage implements \H5PFileStorage {
    ...
    public function saveContent($source, $content) {
        ...
        // Move content folder.
        self::readContentFileTree($source, $options);
    }
    ...
    public function saveFile($file, $contentid, $contextid = null) {
        ...
        $record = get_centralized_db_connection()->get_record_sql("SELECT * FROM `hvp_files` WHERE contenthash = :contenthash",$params);
        
        if ($record === false || !file_exists($CFG->hvp_centralized_filedir.'/'.$checksum[0].$checksum[1].'/'.$checksum.'.'.$file->extension))
        {
            $dirpath = $CFG->hvp_centralized_filedir.'/'.$checksum[0].$checksum[1];
            if (!is_dir($dirpath))
            {
                mkdir($dirpath);
            }
            $filepath = $dirpath.'/'.$checksum.'.'.$file->extension;
            
            move_uploaded_file($tmpfile,$filepath);
            get_centralized_db_connection()->execute("INSERT INTO `hvp_files`(contenthash,filearea,filetype,filename,userid,filesize,timecreated) VALUES(:contenthash,:filearea,:filetype,:filename,:userid,:filesize,:timecreated) ON DUPLICATE KEY UPDATE filename=:filename2",$params);
        }else{
            get_centralized_db_connection()->execute("UPDATE `hvp_files` SET `filename` = :filename WHERE `contenthash`=:contenthash",$params);
        }
        
        return $checksum.'.'.$file->extension;
    }
    ...
    /**
     * Copies files from tmp folder to M@gistere centralized storage.
     *
     * @param string $source
     *  Path to source directory
     * @param array $options
     ...
     */
    // @codingStandardsIgnoreLine
    private static function readContentFileTree($source, $options) {
        $dir = opendir($source);
        if ($dir === false) {
            trigger_error('Unable to open directory ' . $source, E_USER_WARNING);
            throw new \Exception('unabletocopy');
        ...
        while (false !== ($file = readdir($dir))) {
            if (($file != '.') && ($file != '..') && $file != '.git' && $file != '.gitignore') {
                if (is_dir($source . DIRECTORY_SEPARATOR . $file)) {
                    $suboptions = $options;
                    $suboptions['filepath'] .= $file . '/';
                    self::readContentFileTree($source . '/' . $file, $suboptions);
                } else {
                    ...
                    self::add_centralized_file($params,$filepath);
    ...
    private static function add_centralized_file($params,$path)
    {
        global $USER, $CFG;
        /*
        $params = array(
        ...
        */
        ...
        $params['contenthash'] = $checksum;
        $params['filename2'] = $params['filename'];
        
        $record = get_centralized_db_connection()->get_record_sql("SELECT * FROM `hvp_files` WHERE contenthash = :contenthash",$params);
        if ($record === false || !file_exists($CFG->hvp_centralized_filedir.'/'.$checksum[0].$checksum[1].'/'.$checksum.'.'.$params['fileextension']))
        {
            $dirpath = $CFG->hvp_centralized_filedir.'/'.$checksum[0].$checksum[1];
            if (!is_dir($dirpath))
            {
            ...
            $filepath = $dirpath.'/'.$checksum.'.'.$params['fileextension'];
        ...
        return $checksum.'.'.$params['fileextension'];
    }
    ...
    public function moveContentDirectory($source, $contentid = null) {
        ...
        foreach ($contentfiles as $file) {
            if (is_dir("{$contentsource}/{$file}")) {
                self::moveContentFileTree("{$contentsource}/{$file}", $contextid, $contentid);
            } else {
                self::moveContentFile("{$contentsource}/{$file}", $contextid, $contentid);
            }
        }
     ...
     */
    // @codingStandardsIgnoreLine
    private static function moveContentFile($sourcefile, $contextid, $contentid) {
        ...
        $params = array(
            'filearea'    => 'content',
            'filetype'    => finfo_file($finfo, $sourcefile),
            'fileextension' => $extension,
            'filename'    => $filename,
            'timecreated' => time()
        );
        self::add_centralized_file($params,$sourcefile);
    }
     ...
     */
    // @codingStandardsIgnoreLine
    private static function moveContentFileTree($sourcefiletree, $contextid, $contentid) {
        ...
        while (false !== ($file = readdir($dir))) {
            if (($file != '.') && ($file != '..') && $file != '.git' && $file != '.gitignore') {
                if (is_dir("{$sourcefiletree}/{$file}")) {
                    self::moveContentFileTree("{$sourcefiletree}/{$file}", $contextid, $contentid);
                } else {
                    self::moveContentFile("{$sourcefiletree}/{$file}", $contextid, $contentid);
                }
            }
        }
        closedir($dir);
