...
function hvp_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, $options = array()) {
    ...
    switch ($filearea) {
        ...
        case 'editor':
            ...
            $filename = $args[0];
            $checksum = substr($filename, 0 , strrpos($filename, "."));
            if (strlen($checksum) == 32) {
                
            
                $file = get_centralized_db_connection()->get_record_sql("SELECT * FROM `hvp_files` WHERE `contenthash` = ?",array($checksum));
                if ($file)
                {
                    get_centralized_db_connection()->execute("UPDATE `hvp_files` set `timeaccess` = :time, `views`=`views`+1 WHERE contenthash=:checksum",array('time'=>time(),'checksum'=>$checksum));
