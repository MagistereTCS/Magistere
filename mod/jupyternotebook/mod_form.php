...
class mod_jupyternotebook_mod_form extends moodleform_mod {
    const DEFAULT_SERVERURL = "https://nbhosting.inria.fr/ipythonExercice/magistere-lycee";
    ...
    function definition() {
        ...
        $mform->addElement('filemanager', 'files', get_string('selectfile'), null, $filemanager_options);

        $mform->addElement('hidden', 'serverurl');
        $mform->setType('serverurl', PARAM_TEXT);
        $mform->setDefault('serverurl', self::DEFAULT_SERVERURL."/".$CFG->academie_name);
    ...
    function validation($data, $files) {
        ...
        $jupytercommit->commitandpush($CFG->academie_name."/".$PAGE->course->id."/".$filename,$file->get_content(),get_string('commitmessage', 'jupyternotebook', ['name' => $filename, 'activity' => $data["name"],'course' => $PAGE->course->fullname]));
