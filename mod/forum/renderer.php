...
class mod_forum_renderer extends plugin_renderer_base {
    ...
    public function aardvark_custom_section_content($cm){
        ...
        $course_modules = $DB->get_record('course_modules', array('id' => $cm->id));

        if ($forum and has_capability('mod/forum:viewdiscussion', $context)) {

            ob_start();
            if ($course_modules->showdescription && trim($forum->intro) != '') {
                $options = new stdClass();
                $options->para = false;
                echo format_module_intro('forum', $forum, $cm->id);
            }

            forum_section_print_latest_discussions($COURSE, $forum, $COURSE->newsitems);
