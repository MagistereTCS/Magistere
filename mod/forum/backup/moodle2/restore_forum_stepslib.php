...
class restore_forum_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        if ($userinfo) {
            ...
            $paths[] = new restore_path_element('forum_favorite', '/activity/forum/discussions/discussion/posts/post/favorites/favorite');
            $paths[] = new restore_path_element('forum_popularity', '/activity/forum/discussions/discussion/posts/post/popularities/popularity');
        }
    ...
    protected function process_forum_favorite($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->post = $this->get_mappingid('forum_post', $data->post);
        $data->userid = $this->get_mappingid('user', $data->userid);
        $data->timecreated = $this->apply_date_offset($data->timecreated);

        $newitemid = $DB->insert_record('forum_favorites', $data);
    }

    protected function process_forum_popularity($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('forum_popularities', $data);
    }
