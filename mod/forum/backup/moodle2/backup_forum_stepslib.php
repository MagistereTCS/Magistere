...
class backup_forum_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $post->add_child($favorites);
        $favorites->add_child($favorite);

        $post->add_child($popularities);
        ...
        if ($userinfo) {
			...
			$favorite->set_source_table('forum_favorites', array('post' => backup::VAR_PARENTID));
            $popularity->set_source_table('forum_popularities', array('post' => backup::VAR_PARENTID));
