...
function forum_get_completion_state($course,$cm,$userid,$type) {
    ...
    $postcountparams=array('userid'=>$userid,'forumid'=>$forum->id,'completionpopularity'=>$forum->completionpopularity);
    $postcountsql="
SELECT
    COUNT(1)
FROM
...
WHERE
    ...
    if ($forum->completionpopularity) {
        $popularitycountsql = "SELECT COUNT(*) AS nb FROM {forum_posts} fp WHERE (SELECT COUNT(*) FROM {forum_popularities} pop WHERE pop.post = fp.id) >= :completionpopularity AND fp.discussion IN (SELECT id FROM {forum_discussions} WHERE forum = :forumid) AND fp.userid = :userid";
        $value = $DB->get_record_sql($popularitycountsql,$postcountparams)->nb > 0;
        if ($type == COMPLETION_AND) {
            $result = $result && $value;
        } else {
            $result = $result || $value;
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (!forum_user_can_see_post($forum, $discussion, $post, null, $cm, false)) {
        ...
        $output .= html_writer::start_tag('div', array('class'=>'row header none'));
//        $output .= html_writer::tag('div', '', array('class'=>'left picture')); // Picture
    ...
    if (empty($str)) {
        $str = new stdClass;
        if($forum->type == 'blog'){
            if(isset($discussion->firstpost) && $discussion->firstpost == $post->id){
                $str->reply = get_string('replyblogfirstpost', 'forum');
            }else{
                $str->reply = get_string('replyblog', 'forum', $post->firstname . ' ' . $post->lastname);
            }
        }else{
...
//    }
    ...
    if ($forum->type == 'single' and (isset($discussion->firstpost) && $discussion->firstpost == $post->id)) {
...
//    }
    $topicclass = (isset($post->cssclass) ? ' '.$post->cssclass : '');
    ...
    if (!empty($post->discussion)) {
        $bestpopularity = forum_get_best_popularity_post($post->discussion);

        if($bestpopularity && $post->id == $bestpopularity->id){
            $forumpostclass .= ' best-popularity';
        }
    }
    ...
    $output .= html_writer::start_tag('div', array('class'=>'row header'));
    $output .= html_writer::start_tag('div', array('class'=>'picture'));
    ...
    $links = html_writer::link(new moodle_url('post.php', array('quote'=>1,'reply'=>$post->id)), get_string("quotecontribution", "socialforum"), array('class'=>'quote')) . " | ";
    $links .= forum_favorite_button($post->id);
    $links .= ' |';
    $links .= forum_popularity_button($post->id);
    //$output .= html_writer::div($links,'links');
    $output .= html_writer::start_span('links').$links.html_writer::end_span();
    ...
    if ($post->parent == 0)
    {
    	/*$output .= html_writer::tag('div', $postsubject, array('class'=>'subject',
    			'role' => 'heading',
    			'aria-level' => '2'));*/
    	$output .= html_writer::start_span('', array('class'=>'subject', 'role' => 'heading', 'aria-level' => '2'))
                .$postsubject.html_writer::end_span();
    }
    ...
    $output .= html_writer::tag('div', get_string('bynameondate_topic', 'forum', $by), array('class'=>'author',
    		'role' => 'heading',
    		'aria-level' => '2'));
...
function forum_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
    ...
    echo '<td class="popularities">';
    echo $post->popularities;
    echo "</td>\n";
...
function forum_delete_post($post, $children, $course, $cm, $forum, $skipcompletion=false) {
    ...
    if ($DB->delete_records("forum_posts", array("id" => $post->id))) {
        ...
        forum_delete_all_popularity_by_post($post->id);
...
function forum_user_can_see_post($forum, $discussion, $post, $user = null, $cm = null, $checkdeleted = true) {
    ...
    if ($forum->type == 'qanda') {
        ...
        return (($userfirstpost !== false && (time() - $userfirstpost >= $CFG->maxeditingtime)) ||
            (isset($firstpost->id) && $firstpost->id == $post->id) || $post->userid == $user->id || (isset($firstpost->userid) && $firstpost->userid == $user->id) ||
...
// If we want paging
    ...
    if ($displayformat == 'header') {
        ...
        echo '<th class="header popularities" scope="col">'.get_string('popularity', 'forum').'</th>';
        echo '<th class="header favorites" scope="col">'.get_string('favorite', 'forum').'</th>';
    ...
    foreach ($discussions as $discussion) {
        ...
        $popularities = forum_get_all_popularities_by_discussion($discussion->discussion);
        $discussion->popularities = 0;
        if (!empty($popularities)) {
            $discussion->popularities = count($popularities);
        }

        $favorites = forum_get_all_favorites_by_discussion($discussion->discussion);
        $discussion->favorites = 0;
        if (!empty($favorites)) {
            $discussion->favorites = count($favorites);
        }
    ...
    if ($olddiscussionlink && $forum->type !== 'eachuser') {
        if ($forum->type == 'news') {
...
function forum_print_posts_nested($course, &$cm, $forum, $discussion, $parent, $reply, $forumtracked, $posts) {
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            $post->cssclass = 'firstpost starter';
...
// TCS - START
function forum_section_print_latest_discussions($course, $forum, $maxdiscussions = -1, $displayformat = 'plain', $sort = '',
                                        $currentgroup = -1, $groupmode = -1, $page = -1, $perpage = 100, $cm = null) {
 ...
 // Sort out some defaults
    ...
    // if forum is blog, get full post
    // $fullpost = ($forum->type == 'blog');
...
// Decide if current user is allowed to see ALL the current discussions or not
    ...
    if ($PAGE->bodyid == 'page-course-view-topics'){
        if ($canstart and $forum->type == 'eachuser') {
            echo '<div class="singlebutton forumaddnew">';
            echo "<form id=\"newdiscussionform\" method=\"get\" action=\"$CFG->wwwroot/mod/forum/post.php\">";
            echo '<div>';
            echo "<input type=\"hidden\" name=\"forum\" value=\"$forum->id\" />";
...
// Get all the recent discussions we're allowed to see
    ...
    $getuserlastmodified = ($forum->type != 'blog');
...
// If we want paging
    ...
    if($forum->type != 'blog') {
        echo '<table cellspacing="0" class="aard_section_forumheaderlist">';
        echo '<thead>';
        echo '<tr>';
        echo '<th class="text-uppercase picture-header" scope="col" colspan="2">' . get_string('subject_section_header', 'forum') . '</th>';
        // echo '<th class="header author" colspan="2" scope="col">'.get_string('startedby', 'forum').'</th>';

        if (has_capability('mod/forum:viewdiscussion', $context)) {
            echo '<th class="text-uppercase replies-header" scope="col">' . get_string('replies', 'forum') . '</th>';
            // If the forum can be tracked, display the unread column.
            if ($cantrack) {
                echo '<th class="header replies" scope="col">' . get_string('unread', 'forum');
                if ($forumtracked) {
        ...
        foreach ($discussions as $discussion) {
            ...
            forum_section_print_discussion_header($discussion, $forum, $group, $strdatestring, $cantrack, $forumtracked,
                $canviewparticipants, $context, $canviewhiddentimedposts);
        ...
        foreach ($discussions as $discussion) {
            ...
            forum_section_print_discussion_blog($discussion, $forum, $context);
    ...
    if ($page != -1) { ///Show the paging bar
        $pb = new paging_bar($numdiscussions, $page, $perpage, "view.php?f=$forum->id");
        $pb->prepare($OUTPUT, $PAGE, null);
        ...
        if($page > 0){
            $purl = new moodle_url('/mod/forum/view.php', array('f' => $forum->id, 'page' => $page-1));
            $prevarrow = '<i class="fa fa-play fa-flip-horizontal" aria-hidden="true"></i>';
            $previous = html_writer::tag('a', $prevarrow.get_string('topicspreviouslink', 'mod_forum'), array('href' => $purl));
        }

        if($page < ceil($pb->totalcount/$perpage)-1){
            $nurl = new moodle_url('/mod/forum/view.php', array('f' => $forum->id, 'page' => $page+1));
            $nextarrow = '<i class="fa fa-play" aria-hidden="true"></i>';
            $next = html_writer::tag('a', get_string('topicsnextlink', 'mod_forum').$nextarrow, array('href' => $nurl));
        }

        echo html_writer::div($previous, 'left');

        $pagebar = '';
        foreach($pb->pagelinks as $link){
            $pagebar .= $link;
        }

        echo html_writer::div($pagebar, 'center');

        echo html_writer::div($next, 'right');;

        echo '</div>';
    }
}

function forum_section_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
                                        $cantrack = true, $forumtracked = true, $canviewparticipants = true, $modcontext = null,
                                        $canviewhiddentimedposts = false) {
    ...
    */
    ...
    echo '<td class="author">';
    echo '<a class="subject-title" href="'.$CFG->wwwroot.'/mod/forum/discuss.php?d='.$post->discussion.'">'.$post->subject.'</a><br/>';
    echo '<a class="author-field" href="'.$CFG->wwwroot.'/user/view.php?id='.$post->userid.'&amp;course='.$forum->course.'">'.$fullname.'</a>';
    echo ' &#x2022; ';
    echo userdate($post->timemodified, "%d %b %Y");
    echo "</td>\n";
        ...
        if ($cantrack) {
            ...
            if ($forumtracked) {
                if ($post->unread > 0) {
                    ...
                    echo '<a title="'.$strmarkalldread.'" href="'.$CFG->wwwroot.'/mod/forum/markposts.php?f='.
                         $forum->id.'&amp;d='.$post->discussion.'&amp;mark=read&amp;returnpage=view.php&amp;sesskey=' . sesskey() . '">' .
                         '<img src="'.$OUTPUT->image_url('t/markasread') . '" class="iconsmall" alt="'.$strmarkalldread.'" /></a>';
                    echo '</span>';
                } else {
                    echo '<span class="read">';
                    echo $post->unread;
    ...
    // In QA forums we check that the user can view participants.
    if ($forum->type !== 'qanda' || $canviewparticipants) {
        echo '<a class="author-field" href="'.$CFG->wwwroot.'/user/view.php?id='.$post->usermodified.'&amp;course='.$forum->course.'">'.
...
function forum_section_print_discussion_blog($post, $forum, $context){
    global $OUTPUT;
    ...
    $userpicture = $OUTPUT->user_picture($postuser, array('courseid'=>$forum->course));
    $picture = html_writer::div($userpicture, 'picture');

    // main content of the post

    // title
    $linktodiscussion = new moodle_url('/mod/forum/discuss.php', array('d' => $post->discussion));
    $title = html_writer::tag('a', $post->subject, array('href' => $linktodiscussion));
    $divtitle = html_writer::div($title, 'title');

    // author and date
    $linktouserprofile = new moodle_url('/user/view.php', array('id' => $post->userid, 'course' => $forum->course));
    $fullname = fullname($postuser, has_capability('moodle/site:viewfullnames', $context));
    $author = html_writer::tag('a', $fullname, array('href' => $linktouserprofile, 'class' => 'author'));
    $date = userdate($post->timemodified, "%d %b %Y, %H:%M");

    $divauthor = html_writer::div($author.' &#x2022; '.$date, 'author-encart');
    ...
    $divmessage = html_writer::div(format_text($post->message, $post->messageformat, $options),'message');

    // replies count
    $replies = get_string('showreplies', 'mod_forum', $post->replies);
    $replieslink = html_writer::tag('a', '<i class="fa fa-comments fa-2x" aria-hidden="true"></i>'.$replies, array('href' => $linktodiscussion));

    $repliesdiv = html_writer::div($replieslink, 'replies');

    $divpost = html_writer::div($divtitle . $divauthor . $divmessage . $repliesdiv, 'post');
    // html
    $html = $picture . $divpost;

    echo html_writer::div($html, 'blog-post');

}

// Gestion bouton like dans les forums

function forum_popularity_button($id){
    $count_popularities = count(forum_get_popularities_by_post_id($id));
    if(forum_get_popularity_by_user_and_post($id)){
        $islike = 1;
        $class_icon = 'fas fa-thumbs-up post_action liked';
    } else {
        $islike = 0;
        $class_icon = 'far fa-thumbs-up post_action';
    }

    $content = html_writer::tag('i','', array('class'=>$class_icon.' forum_ico', 'aria-hidden'=>'true', 'action'=>"like", 'liked'=>$islike, 'postid'=>$id))
                .html_writer::start_span('count').$count_popularities.html_writer::end_span(); //html_writer::div($count_popularities,'count'). " | ";
    //$content = html_writer::div($content,'popularity', array('id'=>'count_post_'.$id));
    return $content;
}

function forum_get_popularities_by_post_id($postid){
    global $DB;
    return $DB->get_records('forum_popularities', array('post'=>$postid));
}

function forum_get_popularity_by_user_and_post($postid){
    global $DB ,$USER;
    return $DB->get_records('forum_popularities', array('post'=>$postid, 'userid'=>$USER->id));
}

function forum_get_all_popularities_by_discussion($discussionid){
    global $DB;

    $postids = "";
    $posts = forum_get_all_discussion_posts($discussionid, 'id');
    if(!$posts){
        return false;
    }
    foreach($posts as $post){
    ...
    return $DB->get_records_sql('SELECT * FROM {forum_popularities} WHERE post IN('.$postids.')');
}

function forum_get_best_popularity_post($discussionid){
    global $DB;
    ...
    $bestlikepost = false;
    if($postids){
        $bestlikepost = $DB->get_record_sql('   SELECT * FROM {forum_posts} 
                                                            WHERE id =(    SELECT post
                                                                            FROM {forum_popularities} 
                                                                            WHERE post IN('.$postids.')
                                                                            GROUP BY post
                                                                            ORDER BY COUNT(*) DESC, timecreated ASC
                                                                            LIMIT 1)
                                                            LIMIT 1');
    }
    return $bestlikepost;
...
function forum_add_popularity($postid){
    global $DB, $USER;

    $popularity = new stdClass();
    $popularity->userid = $USER->id;
    $popularity->post = $postid;
    $popularity->timecreated = time();

    try {
        $DB->insert_record('forum_popularities', $popularity);
    }
    catch (Exception $e){print_r($e);}
}

function forum_delete_popularity($postid){
    global $DB, $USER;

    try {
        $DB->delete_records('forum_popularities', array('post' => $postid, 'userid'=>$USER->id));
    }
    catch (Exception $e){print_r($e);}
}

function forum_delete_all_popularity_by_post($postid){
    global $DB;

    try {
        $DB->delete_records('forum_popularities', array('post' => $postid));
    }
    catch (Exception $e){print_r($e);}
}

// Gestion bouton favorite

function forum_favorite_button($id){
    global $USER;
    $isfavorite = 0;
    if(forum_get_favorite_by_user_and_post($id)){
        $class = 'fa fa-star post_action';
        $isfavorite = 1;
    } else {
        $class = 'far fa-star post_action';
    }
    //return html_writer::div(html_writer::tag('i','', array('class'=>$class, 'aria-hidden'=>'true', 'action'=>"favorite", 'favorited'=>$isfavorite, 'postid'=>$id)),'favorite');
    return html_writer::tag('i','', array('class'=>$class, 'aria-hidden'=>'true', 'action'=>"favorite", 'favorited'=>$isfavorite, 'postid'=>$id));
}

function forum_get_favorites_by_post_id($postid){
    global $DB;
    return $DB->get_records('forum_favorites', array('post'=>$postid));
}

function forum_get_favorite_by_user_and_post($postid){
    global $DB ,$USER;
    return $DB->get_records('forum_favorites', array('post'=>$postid, 'userid'=>$USER->id));
}

function forum_get_all_favorites_by_discussion($discussionid){
    global $DB;
    ...
    return $DB->get_records_sql('SELECT * FROM {forum_favorites} WHERE post IN('.$postids.')');
}

function forum_add_favorite($postid){
    global $DB, $USER;
    ...
    try {
        $DB->insert_record('forum_favorites', $popularity);
    }
    catch (Exception $e){print_r($e);}
}

function forum_delete_favorite($postid){
    global $DB, $USER;

    try {
        $DB->delete_records('forum_favorites', array('post' => $postid, 'userid'=>$USER->id));
    }
    catch (Exception $e){print_r($e);}
}

function forum_delete_all_favorite_by_post($postid){
    global $DB;

    try {
        $DB->delete_records('forum_favorites', array('post' => $postid));
    }
    catch (Exception $e){print_r($e);}
}

function javascript_for_popularity_buttons(){
    global $CFG;

    $ajax_log_url = $CFG->wwwroot.'/mod/forum/js/ajax.php';
    return ' $( document ).ready(function() {
                    $(".fas.fa-thumbs-up.post_action").click(managePopularity);
                    $(".far.fa-thumbs-up.post_action").click(managePopularity);
                    $(".fa-star.post_action").click(manageFavorite);
                    $(".fa-star-o.post_action").click(manageFavorite);
                });

                function managePopularity(){
                        var obj = $(this);
                        var like = obj.parent(".like");
                        var count = obj.parent().find(".count");
                        $.ajax({
                            type: "POST",
                            url: "'.$ajax_log_url.'",
                            data:{ 
                                postid: obj.attr("postid"),
                        ...
                        $.ajax({
                            ...
                            success:function(response){
                                ...
                                if(obj.attr("favorited") == "1"){
                                    obj.attr("class","fa fa-star post_action");
                                } else {
                                    obj.attr("class","far fa-star post_action");
                                }                               
