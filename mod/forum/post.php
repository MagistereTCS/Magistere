...
$page_params = array('reply'=>$reply, 'forum'=>$forum, 'edit'=>$edit, 'quote'=>$quote);
...
if (!empty($forum)) {      // User is starting a new discussion in a forum
    ...
    if (!empty($confirm) && confirm_sesskey()) {    // User has confirmed the delete
        ...
        if ($replycount) {
            ...
            $heading = $OUTPUT->heading(format_string($forum->name), 2);
...
if($quote){
    $postid = $parent->id;
    $draftid_editor = file_get_submitted_draft_itemid('message');
    $currenttext = file_prepare_draft_area($draftid_editor, $modcontext->id, 'mod_forum', 'post', $postid, mod_forum_post_form::editor_options($modcontext, $postid), $parent->message);
    $userfrom = $DB->get_record('user',array('id' => $parent->userid));
