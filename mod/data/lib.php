...
function data_add_instance($data, $mform = null) {
    ...
    /* => CLEO evol 1672 - 29/03/2018 */
    foreach($data->roles as $roleid){
        $dataNotif = (object) array("recordid" => $data->id, "roleid" => (int) $roleid);
        $DB->insert_record('data_notif_roles', $dataNotif);
    }
    /* <= CLEO 1672 */
...
function data_update_instance($data) {
    ...
    /* => CLEO evol 1672 - 29/03/2018 */
    $DB->delete_records_list('data_notif_roles', 'recordid', array(0 => $data->id));
    foreach($data->roles as $roleid){
        $dataNotif = (object) array("recordid" => $data->id, "roleid" => (int) $roleid);
        $DB->insert_record('data_notif_roles', $dataNotif);
    }
