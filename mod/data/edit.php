...
if ($datarecord = data_submitted() and confirm_sesskey()) {
    if ($rid) {
        ...
        if ($processeddata->validated && $recordid = data_add_record($data, $currentgroup)) {
            ...
            add_to_progress_activity($course->id, $cm->id, -1, 'add');
            //TCS END 2015/02/03 - 585

            /* => CLEO 1672 - 30/03/2018
             Send an email to the registered users with special roles if notifications are requested*/
            if ($data->notifrecord) {
                ...
                if($groupmode != NOGROUPS){
                    $groups = groups_get_user_groups($data->course);

                    $groups = $groups[$cm->groupingid];

                    $groupsql = implode(' AND gm.groupid=', $groups);
                    $groupinner = 'LEFT JOIN {groups_members} gm ON gm.userid=us.id';

                    // find all users who get the moodle/site:accessallgroup
                    $suser = [];
                    foreach(enrol_get_course_users($data->course) as $user){
                    ...
                    if(count($suser)){
                        $specialusers = 'us.id='.implode(' OR us.id=', $suser);

                        // if the user is not in the groups and he is a special (formatateur) one
                        // OR the user belong to one of our group
                        $groupsql = ' AND ('.$specialusers.' OR (gm.groupid='.$groupsql.'))';
                    }else{
                        // add the missing AND
                        $groupsql = ' AND gm.groupid='.$groupsql;
                    }

                }

                $notifUsers = $DB->get_records_sql('
SELECT us.id
FROM {context} con  
INNER JOIN {role_assignments} ra ON con.id = ra.contextid
INNER JOIN {user} us ON us.id = ra.userid
'.$groupinner.'
WHERE ra.roleid IN (SELECT ro.id FROM {role} ro INNER JOIN {data_notif_roles} dnr ON ro.id = dnr.roleid WHERE dnr.recordid = ?)
AND con.instanceid = ? AND con.contextlevel = 50 '.$groupsql.'
GROUP BY us.email', array($data->id, $data->course));

                $params = new \stdClass();
                $params->bdd = $cm->name;
                $params->course = $course->fullname;

                $params->data_link = (new moodle_url('/mod/data/view.php', array('d' => $data->id)))->out();
                $params->record_link = (new moodle_url('/mod/data/view.php', array('d' => $data->id,'rid' =>$recordid)))->out();
                $userfrom = \core_user::get_noreply_user();

                foreach($notifUsers as $notif){
                    if($notif->id == $USER->id){
                        continue;
                    }

                    $userto = $DB->get_record('user', array('id' => $notif->id));

                    $params->data_creator_name = $USER->firstname . " " .  $USER->lastname;
                    $params->userto = $userto->firstname . " " .  $userto->lastname;

                    $messageText = get_string('notifcontenttxtrec', 'data', $params);
                    $messageHTML = get_string('notifcontenthtmlrec', 'data', $params);
                    $subject = get_string('notifsubjectrec', 'data', $params);
...
echo $OUTPUT->header();
//TCS BEGIN 2015/11/16 - NNE
$heading = $OUTPUT->heading(format_string($data->name), 2);
