...
class core_tag_manage_table extends table_sql {
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $sql = "
            SELECT tg.id, tg.academy, tg.name, tg.rawname, tg.isstandard, tg.flag, tg.timemodified,
        ...
        foreach($this->rawdata AS $tagid=>$tag)
        {
        	$tag->otheraca = false;
        	if ($tag->academy != $CFG->academie_name && strlen($tag->academy) > 1 && array_key_exists($tag->academy, get_magistere_academy_config()))
        	{
        		$owner = databaseConnection::instance()->get($tag->academy)->get_record('user',array('id'=>$tag->owner));
        		
        		$tag->otheraca = true;
        		$tag->firstname = $owner->firstname;
        		$tag->lastname = $owner->lastname;
        		$tag->middlename = $owner->middlename;
        		$tag->alternatename = $owner->alternatename;
        		$tag->firstnamephonetic = $owner->firstnamephonetic;
        		$tag->lastnamephonetic = $owner->lastnamephonetic;
        	}
        }
    }
    ...
    public function col_fullname($tag) {
        ...
        if ($tag->otheraca)
        {
        	$ownerlink      = new moodle_url($CFG->magistere_domaine.'/'.$tag->academy.'/user/view.php', $params);
        }else{
