...
class core_tag_collection {
    ...
    public static function get_collections($onlysearchable = false) {
        ...
        if (($tagcolls = $cache->get('tag_coll')) === false) {
            ...
            if (empty($tagcolls)) {
            	...
            	get_centralized_db_connection()->insert_record('tag_coll', array('isdefault' => 1, 'sortorder' => 0));
                ...
                foreach ($tagcolls as $id => $tagcoll) {
                    if ($tagcoll->sortorder != $idx) {
                    	get_centralized_db_connection()->update_record('tag_coll', array('sortorder' => $idx, 'id' => $id));
    ...
    public static function create($data) {
        ...
        $tagcoll->id = get_centralized_db_connection()->insert_record('tag_coll', $tagcoll);
    ...
    public static function update($tagcoll, $data) {
        ...
        get_centralized_db_connection()->update_record('tag_coll', $updatedata);
    ...
    public static function delete($tagcoll) {
        ...
        core_tag_tag::delete_tags(get_centralized_db_connection()->get_fieldset_select('tag', 'id', 'tagcollid = ?', array($tagcoll->id)));

        // Delete the tag collection.
        get_centralized_db_connection()->delete_records('tag_coll', array('id' => $tagcoll->id));
    ...
    public static function change_sortorder($tagcoll, $direction) {
        ...
        get_centralized_db_connection()->update_record('tag_coll', array('id' => $tagcoll->id, 'sortorder' => $idx + $direction));
        get_centralized_db_connection()->update_record('tag_coll', array('id' => $otherid, 'sortorder' => $idx));
        // Reset cache.
        cache::make('core', 'tags')->delete('tag_coll');
        return true;
    }
    ...
    public static function get_tag_cloud($tagcollid, $isstandard = false, $limit = 150, $sort = 'name',
        ...
        if ($rec && $context->contextlevel != CONTEXT_SYSTEM) {
            ...
            if(isset($ids) && !empty($ids) && strlen($ids) > 0){
                $whereclause .= ' AND ctx.path LIKE ? OR (ti.itemtype = "user" AND ti.itemid IN('.$ids.')) OR (tg.id = ti.tagid AND p.id = ti.itemid AND ti.itemtype = "post" AND ba.contextid = '.$ctx.' AND p.id = ba.blogid)';
            } else {
                $whereclause .= ' AND ctx.path LIKE ? OR (tg.id = ti.tagid AND p.id = ti.itemid AND ti.itemtype = "post" AND ba.contextid = '.$ctx.' AND p.id = ba.blogid)';
            }
        ...
        //TCS BEGIN 2018/01/30 NNE
        $from = '';
        $where = '';
        if($context->contextlevel == CONTEXT_COURSE){
            $from = 'INNER JOIN mdl_user_enrolments ue ON ue.userid=u.id
			         INNER JOIN mdl_enrol e ON e.id=ue.enrolid ';

            $where = 'WHERE e.courseid='.$context->instanceid;
        }

        $params = array_merge($params, $params);
        $tagsincloud = $DB->get_records_sql(
"SELECT tags.*, SUM(tags.total) count
FROM (
		SELECT t.id, ti.itemtype, t.rawname, t.name, t.isstandard, t.flag, t.tagcollid, COUNT(t.id) total
		...
		AND ti.component='core' AND ti.itemtype='user'
		GROUP BY t.id
	UNION
		SELECT t.id, ti.itemtype, t.rawname, t.name, t.isstandard,  t.flag, t.tagcollid, COUNT(t.id) total
		FROM mdl_tag_instance ti
		INNER JOIN mdl_tag t ON t.id=ti.tagid
		INNER JOIN mdl_context c ON c.id=ti.contextid
		WHERE 
		c.instanceid IN 
			(
			SELECT u.id
		...
		AND ti.component='core' AND ti.itemtype='post'
		AND c.contextlevel=30
		GROUP BY t.id
) tags
GROUP BY tags.id",
            $params, 0, $limit);
        //TCS END 2018/01/30 NNE
