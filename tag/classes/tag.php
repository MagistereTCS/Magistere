...
class core_tag_tag {
    ...
    protected function __construct($record) {
        ...
        $this->CDB = get_centralized_db_connection();
        $this->record = $record;
    }
    ...
    protected static function add($tagcollid, $tags, $isstandard = false) {
        ...
        foreach ($tags as $veryrawname) {
            ...
            if (!$rawname) {
                ...
                $obj->rawname = $rawname;
                $obj->academy = $CFG->academie_name;
                $obj->name    = core_text::strtolower($rawname);
                $obj->id      = get_centralized_db_connection()->insert_record('tag', $obj);
    ...
    public function update($data) {
        ...
        $data['id'] = $this->id;
        $data['timemodified'] = time();
        $this->CDB->update_record('tag', $data);
    ...
    public function flag() {
        ...
        $this->flag++;
        $this->CDB->update_record('tag', array('timemodified' => $this->timemodified,
    ...
    public function get_tagged_items($component, $itemtype, $limitfrom = '', $limitnum = '', $subquery = '', $params = array()) {
        ...
        $hasfrom = false;
        $courseid = 00;
        $from = optional_param('ctx', null, PARAM_INT);
        if ($from != null)
        {
            $course_ctx = context::instance_by_id($from,IGNORE_MISSING);
            if ($course_ctx !== false && $course_ctx->contextlevel == 50)
            {
                $hasfrom = true;
                $courseid = $course_ctx->instanceid;
            }
        }
        ////// => TCS - JBL Mantis : 2113 - 14/09/2017
        if ($hasfrom && $itemtype == 'user')
        {
            $query = "SELECT it.*
                      FROM {user} it
                      INNER JOIN {tag_instance} ti ON it.id = ti.itemid
                      WHERE ti.itemtype = :itemtype AND ti.tagid = :tagid AND it.id IN
        ...
        if ($component) {
            $query .= ' AND ti.component = :component';
            $params['component'] = $component;
        }
        if ($subquery) {
            $query .= ' AND ' . $subquery;
...
//        }
    ...
    public static function delete_tags($tagids) {
        ...
        {
        	// Skip all non ac-
        	if(substr($academy, 0, 3) != "ac-" && !in_array($academy,array('dgesco','dne-foad','ih2ef','efe','reseau-canope'))){ continue; }
        	...
        	$acaDB->execute($sql, $tagparams);
        }
        // END 2269

        // Delete all the tag correlations.
        $sql = "DELETE FROM {tag_correlation} $select";
        get_centralized_db_connection()->execute($sql, $tagparams);
    ...
    protected function combine_correlated_tags($tags) {
        ...
        if (isset($records[$this->id])) {
        	$this->CDB->update_record('tag_correlation', array('id' => $records[$this->id]->id, 'correlatedtags' => $newcorrelatedlist));
        } else {
        	$this->CDB->insert_record('tag_correlation', array('tagid' => $this->id, 'correlatedtags' => $newcorrelatedlist));
        }
        ...
        foreach ($correlated as $tagid) {
            if (isset($records[$tagid])) {
                ...
                $this->CDB->update_record('tag_correlation', array('id' => $records[$tagid]->id, 'correlatedtags' => $newcorrelatedlist));
            } else {
            	$this->CDB->insert_record('tag_correlation', array('tagid' => $tagid, 'correlatedtags' => '' . $this->id));
            }
        }
    }
    ...
    public function combine_tags($tags) {
        ...
        foreach ($tagstocombine as $tag) {
        	...
        	foreach ($list_academy as $academy => $academyinfo)
        	{
        		// Skip all non ac-
        		if(substr($academy, 0, 3) != "ac-" && !in_array($academy,array('dgesco','dne-foad','ih2ef','efe','reseau-canope'))){ continue; }
        		if ($academy != 'ac-aix-marseille' && $academy != 'ih2ef'){ continue; }
	            ...
	            $mainsql = 'SELECT ti.*, t.name, t.rawname, tim.id AS alreadyhasmaintag '
	                    ...
	                    $records = $acaDB->get_records_sql($mainsql, $params);
	            foreach ($records as $record) {
	                if ($record->alreadyhasmaintag) {
	                	...
	                	$acaDB->delete_records('tag_instance', array('id' => $record->id));
	                    $sql = "UPDATE {tag_instance} SET ordering = ordering - 1
	                            WHERE itemtype = :itemtype
	                        AND itemid = :itemid AND component = :component AND tiuserid = :tiuserid
	                        AND ordering > :ordering";
	                    $acaDB->execute($sql, (array)$record);
	                } else {
	                	...
	                	$acaDB->update_record('tag_instance', array('id' => $record->id, 'tagid' => $this->id));
