...
class tagindex implements templatable {
    ...
    public function __construct($tag, $component, $itemtype, $content,
        ...
        if ($exclusivemode) {
            $this->record->title = get_string('tcsitemstaggedwith', 'tag');
        } else {
        ...
        if($itemtype == "user"){
            $this->record->exclusivetext = get_string('exclusivemodeinterest', 'tag');
        } else {
            $this->record->exclusivetext = get_string('exclusivemode', 'tag');
        }
