...
if ($stage & restore_ui::STAGE_CONFIRM + restore_ui::STAGE_DESTINATION) {
    ...
    if (!$rc) {
        ...
        if ($restore->process()) {
        	...
        	// BEGIN VSE
            $publishid = mmcached_get(str_replace('%MMID%', get_mmid(), $CFG->restoreHubCourseIDKey));
            if ($publishid !== false && $publishid > 0) {
                require_once($CFG->dirroot.'/local/coursehub/CourseHub.php');
            	$hub = CourseHub::instance();
            	$hub->copyIndexationFromMaster($publishid, $restore->get_course_id());
            }
        	// END VSE
        	
        	//on ajoute la methode d'inscription "manuelle" pour ce cours, si la methode n'est pas deja enregistree (MRO)
        	if($restore->get_course_id())
        	{
        		$test_enrol = $DB->get_record('enrol', array('courseid' => $restore->get_course_id(), 'enrol' => 'manual'));
        		if(!$test_enrol)
        		{
        			$role_test = $DB->get_record('role', array('shortname' => 'participant'));
        			$instance_enrol = new stdClass();
        			$instance_enrol->id       = null;
        			$instance_enrol->enrol    = 'manual';
        			$instance_enrol->courseid = $restore->get_course_id();
        			$instance_enrol->status       = 0;
        			$instance_enrol->enrolperiod  = 0;
        			$instance_enrol->roleid       = $role_test->id;
        			$instance_enrol->timemodified = time();
        			$DB->insert_record('enrol', $instance_enrol);
        		}
        	}
        	//FIN AJOUT MRO
