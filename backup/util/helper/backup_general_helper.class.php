...
abstract class backup_general_helper extends backup_helper {
    ...
    public static function get_backup_information($tempdir) {
        ...
        //TCS BEGIN 2018/01/29 NNE
        $section_sequence_info = array(); // use to filter some bad activities
        ...
        if (isset($contentsarr['sections']) && isset($contentsarr['sections']['section'])) {
            ...
            foreach ($sectionarr as $section) {
                ...
                //TCS BEGIN 2018/01/29 NNE
                $sectionxml = $CFG->tempdir . '/backup/' . $tempdir . '/'.$section->directory.'/section.xml';
                if(file_exists($sectionxml)){
                    // true restore
                    $xmldata = simplexml_load_file($sectionxml);
                    $section_sequence_info[$section->sectionid] = explode(',', $xmldata->sequence);
                    $filter_activities = true;
                }else if($info->original_wwwroot == $CFG->wwwroot && $DB->record_exists('course', array('id' => $info->original_course_id))){
                    // restore in import mode, so the source is not the xml, but data from db (if on the same site)
                    $dbsection = $DB->get_records('course_sections', array('course' => $info->original_course_id));
                    foreach($dbsection as $s){
                        $section_sequence_info[$s->id] = explode(',', $s->sequence);
                    }
                    $filter_activities = true;
                }
                //TCS END 2018/01/29 NNE
            }
        ...
        if (isset($contentsarr['activities']) && isset($contentsarr['activities']['activity'])) {
            ...
            foreach ($activityarr as $activity) {
                ...
                //TCS BEGIN 2018/01/29 NNE
                if($filter_activities && (!isset($activity->sectionid) || $activity->sectionid == null || in_array($activity->moduleid, $section_sequence_info[$activity->sectionid]) == false)){
                    continue;
                }
                //TCS END 2018/01/29
        ...
        foreach($settingsarr as $setting) {
            switch ($setting['level']) {
                ...
                case 'activity':
                    //TCS BEGIN 2018/01/29 NNE
                    if(isset($info->activities[$setting['activity']])){
