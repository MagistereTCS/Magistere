...
class core_badges_renderer extends plugin_renderer_base {
    ...
    protected function render_issued_badge(issued_badge $ibadge) {
        ...
        if ($USER->id == $userinfo->id && !empty($CFG->enablebadges)) {
            ...
            $link = new moodle_url('/badges/assertion.php', array('b' => $issued['uid']));
            $js = "onclick = \"navigator.clipboard.writeText('".$link."').then(function() { alert('Lien copié avec succès')}) \"";
            $output .= html_writer::div('<button '.$js.'>'. get_string('copylink') . '</button>');
            ...
            $link = new moodle_url('/badges/prettyview.php', array('hash' => $issued['uid']));
            $output .= html_writer::div($this->output->action_link($link,
                get_string('print', 'glossary'),
                new popup_action('click', $link),
                array('class' => 'print-button btn')),'print-action');
        }
    ...
    protected function render_badge_recipients(badge_recipients $recipients) {
        ...
        // TCS - START - 20191115+3611
        // hide the date column for the students or lower
        $ignored_roles = array('visiteur','utilisateurauthentifie','participant');
        $roles = get_user_roles($PAGE->context);
        $canviewdate = false;
        foreach($roles AS $role) {
            if (!in_array($role->shortname, $ignored_roles)) {
                $canviewdate = true;
            }
        }
        ...
        if($canviewdate){
            ...
            $table->head = array($sortbyname, '');
        }
        // TCS - END - 20191115+3611
        ...
        foreach ($recipients->userids as $holder) {
            ...
            // TCS - 20191115
            $awarded  = $canviewdate ? userdate($holder->dateissued) : '';
