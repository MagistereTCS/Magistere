...
if ($delete) {
    ...
    $sql = "SELECT *
              FROM {local_bs_badge_pool_cat} WHERE deleted = 0
          ORDER BY name ".$dir;

    $categories = $DB->get_recordset_sql($sql, null, $start, $perpage);
    $badgescount = $DB->count_records('local_bs_badge_pool_cat',array('deleted'=>0));
    ...
    foreach ($categories as $category) {
        $name = html_writer::div((in_array($category->sourceaca,$CFG->badge_pool_sources)?$category->name.' (national)':$category->name), 'name');
        $description = html_writer::div($category->description, 'description');
        if ($category->sourceaca == $CFG->academie_name) {
