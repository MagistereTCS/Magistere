...
$sql = "SELECT *
          FROM {local_bs_badge_pool_badges} WHERE deleted = 0
      ORDER BY ".$sort." ".$dir;

$badges = $DB->get_recordset_sql($sql, null, $start, $perpage);
$badgescount = $DB->count_records('local_bs_badge_pool_badges',array('deleted'=>0));
...
foreach ($badges as $badge) {
    ...
    $badgecategory = $DB->get_record('local_bs_badge_pool_cat', array('id' => $badge->categoryid));
    ...
    $category = html_writer::div((in_array($badgecategory->sourceaca,$CFG->badge_pool_sources)?$badgecategory->name.' (national)':$badgecategory->name), 'category');
    if ($badge->sourceaca == $CFG->academie_name) {
