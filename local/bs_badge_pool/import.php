...
if (!$data = $form->get_data()) {
    ...
    if (in_array($CFG->academie_name,$CFG->badge_pool_sources) || $category->sourceaca == $CFG->academie_name || is_siteadmin())
    {
    ...
    if (empty($xmlfile)) {
        ...
        foreach ($xmlfile->category as $category) {
            ...
            if ($data->category == 0) {
                ...
                $newcategory->hash = sha1($newcategory->name.$newcategory->description.time());
                $newcategory->sourceaca = $CFG->academie_name;
                $newcategory->updated = 0;
                ...
                $bcat = $DB->get_record('local_bs_badge_pool_cat', array('id'=>$data->category));
                if (!in_array($CFG->academie_name,$CFG->badge_pool_sources) && $bcat->sourceaca != $CFG->academie_name)// && !is_siteadmin())
                {
                    print_error("Vous ne pouvez pas modifier cette catégorie!");
                }
            ...
            foreach ($category->badge as $badge) {
                ...
                $newbadge->updated = 0;
                $newbadge->deleted = 0;
                $newbadge->hash = sha1($newbadge->name.$newbadge->categoryid.time());
                $newbadge->sourceaca = $CFG->academie_name;
