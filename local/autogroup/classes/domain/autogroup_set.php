...
class autogroup_set extends domain
{
    ...
    /**
     * @param string $groupidnumber
     * @param \moodle_database $db
     * @return string
     * @throws \dml_exception
     */
    private function prepare_new_group_id_number($groupidnumber, \moodle_database $db) {
        $sql = "SELECT count(*) as nbgroups".PHP_EOL
            ."FROM {groups} g".PHP_EOL
            ."WHERE g.courseid = :courseid".PHP_EOL
            ."AND ".$db->sql_like('g.idnumber', ':autogrouptag');
            $param = array(
        ...
        $nbRecord = $db->get_records_sql($sql,$param);
        // groupIdNumber must be limited to 100 chars to fit in db, we keep 3 chars for the group number in the autogroup
        $groupNumber = 0;
        if (count($nbRecord) > 0) {
            $groupNumber = key($nbRecord);
        }
        $groupidnumber = substr($groupidnumber,0, 97);
        $groupidnumber .= $groupNumber;
    ...
    private function get_or_create_group_by_idnumber($group, \moodle_database $db)
    {
        ...
        $groupname = substr($groupname, 0, 254); // name is limited to 254 chars in db
        $result = null;
        ...
        $data->id = 0;
        $data->name = $groupname;
        $data->idnumber = $this->prepare_new_group_id_number($idnumber, $db);
