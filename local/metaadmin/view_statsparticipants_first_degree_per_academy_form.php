...
class view_statsparticipants_first_degree_per_academy_form extends moodleform {
    protected $course;
    protected $context;
...
$(function(){
        require(["local_magisterelib/jtable"],function(){
        	...
        	{
        		downloading = true;
        		$("#mform1").attr("action","view_statsparticipants_first_degree_per_academy_export.php?action=export&format=csv&so="+sortorder);
        	...
        	function loadExport(execafter)
        	{
				...
				$.ajax({
					url: "view_statsparticipants_first_degree_per_academy_export.php?action=export&format=htmltsv&so=" + sortorder,
					type: "POST",
					data: postData,
        	...
        	$("#resultTable").jtable({
				...
				actions: {
                    listAction: function (postData, jtParams) {
                        return $.Deferred(function ($dfd) {
                            ...
                            $.ajax({
                                url: " view_statsparticipants_first_degree_per_academy_ajax.php?action=list&so=" + jtParams.jtSorting,
                                type: "POST",
                                dataType: "json",
                                data: postData,
                                success: function (data) {
				...
				fields: {
					...
					dpt_name: {
						title: "'.get_string('jtheader_dpt_name', 'local_metaadmin').'",
						width: "10%",
						create: false,
						edit: false,
						list: true
					...
					dpt_code: {
						title: "'.get_string('jtheader_dpt_code', 'local_metaadmin').'",
						width: "10%",
						create: false,
						edit: false,
						list: true
