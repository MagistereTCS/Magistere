...
$via_id = required_param('vid', PARAM_INT);
...
$startdate = date('d/m/Y',time()-(8*86400));
$enddate = date('d/m/Y',time()+(31*86400));
...
if ($isCanceled) {
    ...
    $data->grouping = '';
    $data->via_id = required_param('via_id', PARAM_INT);
...
$PAGE->requires->js_call_amd("local_gaia/linkVia", "init", array($CFG->wwwroot.'/local/gaia/api.php',$gaia_formation->dispositif_name,$gaia_formation->module_name,$gaia_formation->formation_place));
...
<div id="session_data" style="display:none">
    <form autocomplete="off" action="" method="post" id="form">
        ...
        <input type="hidden" id="via_id" name="via_id" value="'.$via_id.'" />
        ...
        <div id="groupings">
            <label>Associer le groupe à un groupement<br/><span>Attention, seuls les utilisateurs membres du groupement, c\'est à dire membre du groupe associé pourront participer à la classe virtuelle</span></label>
            <select name="grouping" id="grouping_select"></select>
        </div>
...
function process_postdata($data)
{
    ...
    if($data->via_id !== null){
        GaiaUtils::bind_activity_with_gaia($data->via_id, $data->session_id, $data->dispositif_id, $data->module_id);
    }
