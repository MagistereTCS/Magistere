YUI(M.yui.loader, {lang: M.local_mail_lang}).use('io-base', 'node', 'json-parse', 'panel', 'datatable-base', 'dd-plugin', 'moodle-form-dateselector', 'datatype-date', function(Y) {
    ...
    var mail_doaction = function(action, node) {
        ...
        if(mail_message_view) {
            ...
            } else if (action == 'delete' || action == 'restore' || action == 'deletedefinitive') {
	        ...
	        } else if (action == 'deletedefinitive') {
	            mail_undo_function = action;
	            ids = mail_get_checkboxs_values();
            ...
            if (nodes.size()) {
                nodes.each(function (node) {
                    ...
                    } else if(action == 'delete' || action == 'restore' || action == 'discard' || action == 'deletedefinitive') {
                        ancestor.remove();
                    } else if(action == 'assignlabels') {
    ...
    //Delete definitive button
    Y.one("#region-main").delegate('click', function(e) {
        if (!this.hasClass('mail_button_disabled')) {
            mail_doaction('deletedefinitive');
        }
    }, '#mail_deletedefinitive');
