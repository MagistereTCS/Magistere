...
class local_mail_renderer extends plugin_renderer_base {
    ...
    public function delete($trash) {
        ...
        $label = ($trash ? get_string('restore', 'local_mail') : get_string('puttotrash', 'local_mail'));
        $attributes = array(
            'type' => 'submit',
            'name' => 'delete',
            'value' => $label,
    ...
    public function deletedefinitive() {
    	$type = 'deletedefinitive';
    	$label = get_string('delete_definitive', 'local_mail');
    	$attributes = array(
    			'type' => 'submit',
    ...
    public function labels($type) {
        ...
        $output .= html_writer::tag('span', $label);
        $url = $this->output->image_url('t/expanded', 'moodle');
    ...
    public function search() {
        ...
        $output .= html_writer::empty_tag('input', $attributes);
        $output .= html_writer::end_tag('div');
        $url = $this->output->image_url('t/collapsed', 'moodle');
    ...
    public function toolbar($type, $courseid = 0, $params = null) {
        ...
        if ($type === 'reply') {
            ...
            $delete_definitive = '';
            if ($type == 'trash') {
            	$delete_definitive = $this->deletedefinitive();
            }
            $pagingbar = '';
            if ($type !== 'view') {
                if ($type !== 'drafts') {
            ...
            $left = html_writer::tag('div',
                $goback . $selectall . $labels . $read . $unread . $delete . $delete_definitive . $extended . $more . $search . $selectedlbl,
