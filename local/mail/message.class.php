...
class local_mail_message {
    ...
    public function deletedefinitive($messageid) {
    	global $DB, $USER;
    	$transaction = $DB->start_delegated_transaction();
    	$users = $DB->get_records('local_mail_message_users', array('messageid' => $messageid));

    	if(count($users) > 0){   		
    		foreach ($users as $user){
    			if ($user->userid == $USER->id){
    				$DB->delete_records('local_mail_index', array('messageid' => $messageid, 'userid' => $user->userid, 'type' => 'trash'));
    			}  			
    		}
    	}
    	$indexall = $DB->get_records('local_mail_index', array('messageid' => $messageid));
    	if(count($indexall) <= 0){
    		$DB->delete_records('local_mail_messages', array('id' => $messageid));
    		$DB->delete_records('local_mail_message_refs', array('messageid' => $messageid));
    		$DB->delete_records('local_mail_message_users', array('messageid' => $messageid));
    		$DB->delete_records('local_mail_message_labels', array('messageid' => $messageid));
    		$DB->delete_records('local_mail_index', array('messageid' => $messageid));
    	}
    	$transaction->allow_commit();
    }
