...
$backurl     = optional_param('return', $CFG->magistere_domaine . '/' . $CFG->academie_name, PARAM_TEXT);

$url = new moodle_url('/local/mail/view.php', array('t' => $type));
$offset = max(0, $offset);

$PAGE->set_url('/local/mail/view.php');
$PAGE->navbar->add(get_string('mail:navbartitle', 'local_mail'));
...
if ($removelbl) {
    ...
    echo '<div class="activity-encart">
    			<a href="'.$backurl.'" class="activity-encart-back-button">
    					'.get_string('mail:back', 'local_mail').'</a>
    			<hr class="activity-encart-upline" style="display: block;"/></div>';
