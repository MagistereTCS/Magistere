...
abstract class format_section_renderer_base extends plugin_renderer_base {
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        // TCS - NNE - 20180608
        if(($course->format == 'topics' or $course->format == 'magistere_topics') && !(has_capability('moodle/site:manageblocks', context_course::instance($course->id)) && has_capability('moodle/course:update', context_course::instance($course->id))))
        {
    ...
    protected function section_hidden($sectionno, $courseorid = null) {
        if ($courseorid) {
            ...
            $strnotavailable = get_string('notavailablecourse2', '', $sectionname);
        } else {
        ...
        $o.= html_writer::tag('div', '', array('class' => 'right side'));
        $o.= html_writer::start_tag('div', array('class' => 'content'));
        $o.= html_writer::tag('div', $strnotavailable, array('class' => 'alert alert-error alert-block hidden-section-alert'));
        $o.= html_writer::end_tag('div');
        $o.= html_writer::end_tag('li');
        return $o;
    }
