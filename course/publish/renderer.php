...
class core_publish_renderer extends plugin_renderer_base {
     ...
     * @return $html string
     */
    public function sentbackupinfo($id, $huburl, $hubname, $error = false) {
        $html = html_writer::tag('div', get_string('sent', 'hub'),
       	...
       	if(!$error){
	        ...
	        $publishindexurl = new moodle_url('/course/publish/index.php',
	                        array('sesskey' => sesskey(), 'id' => $id,
	                            'error' => true, 'huburl' => $huburl, 'hubname' => $hubname));
       	}
    ...
    /**
     * Display error during uploading backup process
     * @return $html string
     */
    public function errorsendingbackupinfo() {
    	$html = html_writer::tag('div', get_string('errorsendingsize', 'hub'),
    			array('class' => 'errorcourseuploadtextinfo'));
    	return $html;
    }
