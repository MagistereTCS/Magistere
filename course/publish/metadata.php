...
if (has_capability('moodle/course:publish', context_course::instance($id))) {
    ...
    if (!empty($fromform)) {
        ...
        //TCS BEGIN - NNE
        $courseinfo->inscription_method = isset($fromform->inscription_method) ? $fromform->inscription_method : null;
        	
        
        
        $courseinfo->enrolstartdate = 0;
        $courseinfo->enrolenddate = 0;
        $courseinfo->isasession = 0;
        $courseinfo->maxparticipant = 0;
        $courseinfo->deletescreenshots = 0;
        $courseinfo->enrolrole = 0;
        
        
        
        $courseinfo->hasakey = 0;
        if(!empty($fromform->enrolstartdate)){
        	$courseinfo->enrolstartdate = $fromform->enrolstartdate;
        }
        
        if(!empty($fromform->enrolenddate)){
        	$courseinfo->enrolenddate = $fromform->enrolenddate;
        }
        
        if(!empty($fromform->isasession)){
        	$courseinfo->isasession = $fromform->isasession;
        }
        
        if(!empty($fromform->enrolrole)){
        	$courseinfo->enrolrole = $fromform->enrolrole;
        }
        
        if(!empty($fromform->maxparticipant)){
        	$courseinfo->maxparticipant = $fromform->maxparticipant;
        }
        
        if(!empty($fromform->hasakey)){
        	$courseinfo->hasakey = $fromform->hasakey;
        }
        //TCS END - NNE
        ...
        $indexationServices = new IndexationServices();
        $indexationServices->setHubCourseIndexation($id, $courseids[0]);
        // END VSE
