...
class course_publication_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('searchableselector', 'subject', get_string('subject', 'hub'), $options, array('selectablecategories' => false));
        unset($options);
...
//        $this->init_javascript_enhancement('subject', 'smartselect', array('selectablecategories' => false, 'mode' => 'compact'));
        ...
        //TCS BEGIN - NNE
        //add enrol method
        $enrol = $DB->get_records('enrol', array('courseid' => $course->id, 'enrol' => 'self', 'status' => 0,'customint6' => 1));
        
        if($enrol && !empty($enrol)){
        	$inscription_method = array();
        	
        	foreach($enrol as $enr){
        		$inscription_method[] = $enr->enrol;
        		
        		$mform->addElement('hidden', 'enrolstartdate', $enr->enrolstartdate);
        		$mform->addElement('hidden', 'enrolenddate', $enr->enrolenddate);
        		
        		$mform->setType('enrolstartdate', PARAM_INT);
        		$mform->setType('enrolenddate', PARAM_INT);
        			
        		$hasakey = !empty($enr->password) ? 1 : 0;
        			        			 
        		$mform->addElement('hidden', 'hasakey', $hasakey);
        		$mform->setType('hasakey', PARAM_INT);
        			
        		//get the maximum of participant allowed
        		$mform->addElement('hidden', 'maxparticipant', $enr->customint3);
        		$mform->setType('maxparticipant', PARAM_INT);
        			
        		//retreive the role
        		$role = $DB->get_record('role', array('id' => $enr->roleid));
        			 
        		if($role){
        			$mform->addElement('hidden', 'enrolrole', $role->shortname);
        			$mform->setType('enrolrole', PARAM_TEXT);
        		}
        	}
        	
        	$mform->addElement('hidden', 'inscription_method', implode(",", $inscription_method));
        	$mform->setType('inscription_method', PARAM_TEXT);
        }
        
        //check if the course is a Session caterogie or a sub categorie of Session
        $categorie = $DB->get_record_sql('SELECT * FROM {course_categories} WHERE id in (SELECT category FROM {course} WHERE id=' . $course->id . ')');

        if($categorie !== false){
        	$sessionCategorie = $DB->get_record_sql('SELECT * FROM {course_categories} WHERE LCASE(name) like "session en auto-inscription"');
        	...
        	$mform->addElement('hidden', 'isasession', $isasession);
        	$mform->setType('isasession', PARAM_INT);
        }
