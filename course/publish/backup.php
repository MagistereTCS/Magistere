...
$fromcoursepublish = optional_param('fromcoursepublish', false, PARAM_BOOL);
...
$backup = new backup_ui($bc,
        array('id' => $id, 'hubcourseid' => $hubcourseid, 'huburl' => $huburl, 'hubname' => $hubname, 'fromcoursepublish' => $fromcoursepublish));
$backup->process();
...
$errorpublish = false;
if($backupfile->get_filesize() > 536870912){
	$errorpublish=true;
}
...
if(!$errorpublish){
	...
	echo $renderer->errorsendingbackupinfo();
}
...
// BEGIN VSE
	...
	$indexationServices->setHubCourseIndexation($id, $hubcourseid);
// END VSE
}

//AJOUT MRO, ON ENVOI LES DONNEES DINDEXATION AU HUB SERVER
//PBO : OK LOL

if($errorpublish){
	$DB->delete_records('course_published', array('courseid' => $id, 'hubcourseid' => $hubcourseid));
}
