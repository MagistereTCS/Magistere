...
abstract class moodleform_mod extends moodleform {
    ...
    /**
     * #1672 - evol 2.0.10 - CLEO 26/03/2018
     */
    public function add_notifications_management()
    {
        global $DB;
        global $OUTPUT;
        global $CFG;
        ...
        $mform->addElement('header', 'notificationshdr', get_string('notifications', 'data'));
        $mform->addElement('selectyesno', 'notifrecord', get_string('notifrecord', 'data'));

        $tabRoles = $DB->get_records_sql('SELECT r.id, r.name FROM {role} r WHERE r.shortname = "participant" OR r.shortname = "formateur" OR r.shortname = "tuteur"');
        $optionRoles = array();
        foreach($tabRoles as $data){
            $optionRoles[$data->id] = $data->name;
        }

        $mform->addElement('select', 'roles', get_string('usertypes', 'data'), $optionRoles);
        $mform->getElement('roles')->setMultiple(true);

        $mform->addRule('roles', 'fdsqfdsqfsdq', 'required');

        $notifRoles = $DB->get_records_sql('SELECT ro.id, ro.name 
                                            FROM {role} ro 
                                            INNER JOIN {data_notif_roles} dnr ON ro.id = dnr.roleid 
                                            WHERE dnr.recordid = ?', array($this->_cm->instance));
        $mform->getElement('roles')->setSelected(array_keys($notifRoles));
        $mform->disabledIf('roles', 'notifrecord', 'eq', 0);

        $mform->addElement('selectyesno', 'notiffeedback', get_string('notiffeedback', 'data'));
    }
}
