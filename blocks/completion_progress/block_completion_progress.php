...
class block_completion_progress extends block_base {
    ...
    function instance_delete() {
        global $COURSE;
        $event = \block_completion_progress\event\instance_deleted::create(
            array(
                'context' => $this->context,
                'other' => $this->instance->id,
                'courseid' => $COURSE->id,
            ));
        $event->trigger();
    ...
    function instance_config_save($data, $nolongerused = false) {
        ...
        global $COURSE;
        $event = \block_completion_progress\event\config_updated::create(
            array(
                'context' => $this->context,
    ...
    function instance_create() {
        global $COURSE;
        $event = \block_completion_progress\event\instance_created::create(
            array(
                'context' => $this->context,
                'objectid' => $this->instance->id,
                'courseid' => $COURSE->id,
    ...
    public function init() {
        ...
        if($new_link == false){
            ...
            $link = '<a href="https://wiki.magistere.education.fr/Suivi_des_participants" target="_blank" style="color:#fff;"><i class="far fa-question-circle" aria-hidden="true"></i></a>';
            $this->title = get_string('config_default_title', 'block_completion_progress').' '.$link;
        }
    }
    ...
    public function get_content() {
        ...
        if (block_completion_progress_on_site_page()) {
            ...
            if (has_capability('block/completion_progress:overview', $this->context)) {
                ...
                $this->content->text .= HTML_WRITER::div(HTML_WRITER::link($url, $label),'newlink');
            }else{
                $this->content->text .= HTML_WRITER::div("",null,array('style' => 'padding: 0 0 0 25px !important;display: inline-block;'));
            }
        }
        ...
        $this->content->text .= '
                    <script type="text/javascript">
                        $(document).ready(function($) {
                            $(".block_completion_progress").on("mouseout", ".progressBarCell", function(){                                                              
                                $(".block_completion_progress .progressEventInfo:not(:first)").attr("style","display: none;");
                                $(".block_completion_progress .progressEventInfo:first").attr("style","display: block;");
                            });
                        });
                    </script>';
