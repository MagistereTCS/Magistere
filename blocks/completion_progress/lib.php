            ...
            if (
                ...
                && ($config == null
                    || (!isset($config->activitiesincluded)
                        || ($config->activitiesincluded != 'selectedactivities' || in_array($module.'-'.$cm->instance, $config->selectactivities))
                    )
                )
            ) {
				$icon = $cm->get_icon_url();
                if ($module == "educationallabel" || $module == "label") {
                    $iconurl = explode('/', $icon);
                    $moduleurl = $iconurl[0].'//'.$iconurl[2].'/'.$iconurl[3]."/course/view.php?id=".$courseid."&section=".$cm->sectionnum."#module-".$cm->id;
                } else if ($module == 'completionmarker') {
                    $moduleurl = (new moodle_url('/course/view.php',array('id'=>$cm->course,'section'=>$cm->sectionnum)))->out(false);
                } else {
                    $moduleurl = method_exists($cm->url, 'out') ? $cm->url->out() : '';
                }
...
function block_completion_progress_bar($activities, $completions, $config, $userid, $courseid, $instance, $simple = false) {
    ...
    $divoptions = array('class' => 'progressEventInfo default',
    ...
    foreach ($activities as $activity) {
		...
		$content .= HTML_WRITER::div(html_writer::empty_tag('img',
            array('src' => $OUTPUT->image_url('general/icon_progress_activity_'. $activity['type'], 'theme'),
                'class' => 'moduleIcon', 'alt' => '', 'role' => 'presentation')),'progressEventInfo_activity_block');

        $content .= HTML_WRITER::start_div('progressEventInfo_activity_block detail');																		  
        $text = '';
		$text .= HTML_WRITER::span(s($activity['modulename']),'progressEventInfo_activity_title');																						  
		...
		$content .= HTML_WRITER::start_div('status');											 
        $altattribute = '';
