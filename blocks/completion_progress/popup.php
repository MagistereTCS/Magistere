    ...
    <div style="border:1px solid #000000;padding:10px;margin-bottom:15px">
        ...
        <ul style="list-style-type:none;margin-left:100px">
            ...
            if (count($tuteurs)>0)
            {
                ...
                echo '<li>Aucun tuteur trouvé</li>';
            }
            ?>
        </ul>
        <p>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#enddate_datepicker").datepicker("option", "monthNames", ['janvier', 'f&eacute;vrier', 'mars', 'avril', 'mai', 'juin', 'juillet', 'ao&ucirc;t', 'septembre', 'octobre', 'novembre', 'd&eacute;cembre']);
        $("#enddate_datepicker").datepicker("option", "monthNamesShort", ['janv.', 'f&eacute;vr.', 'mars', 'avril', 'mai', 'juin', 'juil.', 'ao&ucirc;t', 'sept.', 'oct.', 'nov.', 'd&eacute;c.']);
        $("#enddate_datepicker").datepicker("option", "dayNames", ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi']);
        $("#enddate_datepicker").datepicker("option", "dayNamesShort", ['dim.', 'lun.', 'mar.', 'mer.', 'jeu.', 'ven.', 'sam.']);
        $("#enddate_datepicker").datepicker("option", "dayNamesMin", ['D', 'L', 'M', 'M', 'J', 'V', 'S']);
        $("#enddate_datepicker").datepicker("option", "dateFormat", 'dd/mm/yy');
        $("#enddate_datepicker").datepicker("option", "weekHeader", 'Sem.');
        $("#enddate_datepicker").datepicker("option", "firstDay", 1);
        $("#enddate_datepicker").datepicker("option", "currentText", 'Aujourd\'hui');
        $("#enddate_datepicker").datepicker("option", "closeText", 'Fermer');
        $("#enddate_datepicker").datepicker("option", "prevText", 'Pr&eacute;c&eacute;dent');
        $("#enddate_datepicker").datepicker("option", "nextText", 'Suivant');
        $("#enddate_datepicker").datepicker("option", "isRTL", false);
        $("#enddate_datepicker").datepicker("option", "showMonthAfterYear", false);
        $("#enddate_datepicker").datepicker("option", "buttonImage", '<?php echo $OUTPUT->image_url('general/calendar','theme'); ?>');
        $("#enddate_datepicker").datepicker("option", "buttonImageOnly", true);
        $("#enddate_datepicker").datepicker("option", "showOn", 'button');
    });
</script>
</div>
