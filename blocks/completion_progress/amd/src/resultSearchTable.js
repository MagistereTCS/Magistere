/* jshint ignore:start */
define(['jquery', 'core/str', 'core/templates', 'core/notification', 'core/ajax', 'local_magisterelib/jtable'], function($, Str, Templates, Notification, Ajax) {
    function init(ajax_url, ajax_redirect, id, contextid, courseid, jtheader_firstname, jtheader_lastname, jtheader_lastlogin,
        ...
        $('#ProgressBarOverviewTable').on('mouseout', '.progressBarCell', function() {
            $(".progressEventInfo:not(:first)").attr("style","display: none;");
        });
        ...
        $('#select_submit').on('change', function (e) {
            ...
            if (valueSelected == 'formation_finished') {
                if (selectedRowsObject == "") {
                	...
                	$('#selected-participants-list ul').empty();
                	$(selectedRowsObject).each(function(key, value) {
                		$('#selected-participants-list ul').append('<li>' + value.firstname + ' ' + value.lastname + '</li>');
                	});
					$( "#dialog-formation-finished" ).dialog({
						resizable: false,
						width: 600,
						height: 'auto',
						maxHeight: 470,
