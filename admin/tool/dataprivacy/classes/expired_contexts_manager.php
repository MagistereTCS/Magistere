...
class expired_contexts_manager {
    ...
    public function flag_expired_contexts() : array {
        ...
        // BEGIN ARO 20/04/2020 3375 Désactivation des flag des parcours pour magistere
        /*
    ...
    protected static function clear_old_records() {
        ...
        // BEGIN ARO 20/04/2020 3375 Simplification de la requête
        $sql = "
            SELECT dpctx.*
            FROM {tool_dataprivacy_ctxexpired} dpctx
            LEFT JOIN {context} c ON dpctx.contextid = c.id
    ...
    protected static function get_nested_expiry_info_from_sql(string $sql, array $params) : array {
        ...
        foreach ($fulllist as $record) {
            ...
            if (!self::is_eligible_for_deletion($pathstoskip, $context)) {
                // BEGIN ARO 20/04/2020 3375 On ignore les contextes utilisateurs avec depth > 2
    ...
    public function process_approved_deletions() : array {
        ...
        $limit = function_exists("get_deletion_expired_user_limit") ? get_deletion_expired_user_limit() : 10;

        $expiredcontexts = expired_context::get_records(['status' => expired_context::STATUS_APPROVED],null,null,null,$limit);
