...
class process_data_request_task extends adhoc_task {
    ...
    public function execute() {
        ...
        if ($deleteuser) {
            ...
            global $DB;
            $record = new stdClass();
            $record->id = $foruser->id;
            $record->firstname =get_config("tool_dataprivacy","anonymousfirstname")."_".$foruser->id;
            $record->lastname = get_config("tool_dataprivacy","anonymouslastname")."_".$foruser->id;
            $record->username =get_config("tool_dataprivacy","anonymoususername")."_".$foruser->id;
            $record->email =get_config("tool_dataprivacy","anonymousemail")."_".$foruser->id."@".get_config("tool_dataprivacy",'anonymousemail').".lan";

            $record->firstaccess =0;
            $record->lastaccess =0;
            $record->lastlogin =0;
            $record->currentlogin =0;
            $record->lastip ="";
