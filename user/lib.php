...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    */
    ...
    // 2017-12-05-TCS-NNE BEGIN 1998
    if ($user->auth == 'shibboleth')
    {
        //$aca = user_get_mainacademy();
        //if ($aca != '' && strlen($aca) > 0)
        //{
        if (!isset($user->profile['codaca']) || (isset($user->profile['codaca']) && (intval($user->profile['codaca']) == 0 || $user->profile['codaca'] == '') ) )
        {
            //$display_reset_aca = true;
            $chgpass = new stdClass();
            $chgpass->itemtype = 'link';
            $chgpass->title = 'Réinitiliser ma plateforme de rattachement';
            $chgpass->url = new moodle_url('/');
            $chgpass->titleidentifier = 'changeaca,moodle';
            $chgpass->pix = 't/preferences';

            $returnobject->navitems[] = $chgpass;
        }
        //}
    }else if($user->auth == 'manual' ){
        $chgpass = new stdClass();
        $chgpass->itemtype = 'link';
        $chgpass->title = 'Changer le mot de passe';
        $chgpass->url = new moodle_url('/login/change_password.php');
        $chgpass->titleidentifier = 'changepassword,moodle';
        $chgpass->pix = 't/preferences';

        $returnobject->navitems[] = $chgpass;
    }
    // 2017-12-05-TCS-NNE END 1998
...
function user_get_tagged_users($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = 1, $page = 0) {
    ...
    if ($usercount) {
        ...
        $fctx = context::instance_by_id($fromctx);
        
        $instanceid = ($fctx->contextlevel == CONTEXT_COURSE ? $fctx->instanceid : $SITE->id);
        ...
        $wherebadge = ($instanceid == $SITE->id ? '' : ' AND b.courseid = :instanceid');

        $query = "SELECT u.*, tuai.appelation_officielle, tuai.ville, tuai.coordonnee_lat, tuai.coordonnee_long, ".$instanceid." courseid,
                    (
                        SELECT COUNT(bi.id)
                        FROM {badge} b 
                        INNER JOIN {badge_issued} bi ON bi.badgeid=b.id
                        WHERE bi.userid=u.id AND (b.status=:active OR b.status=:activelocked)".$wherebadge."
                    ) AS badges
                      FROM {user} u 
        ...
        $userlist2map = $DB->get_records_sql($query, $params,  $page*$perpage, $perpage);


        $content .= userTagMap($userlist2map);
...
/**
 * To create an interactive map of the users sharing a same tag.
 * @param $participantslist
 */
function userTagMap($participantslist)
{
    global $PAGE, $OUTPUT;
    ...
    {
        $profilelink = new moodle_url('/user/view.php', array('id'=>$participant->id));
        $messagelink = new moodle_url('/message/index.php', array('id'=>$participant->id));
        $interests = core_tag_tag::get_item_tags('core', 'user', $participant->id);
        ...
        foreach($interests AS $interest)
        {
            ...
            $tagurl = new moodle_url("/tag/index.php",array('tc'=>1,'tag'=>$interest->name,'from'=>$interest->taginstancecontextid));
            $taglist .= '<li><a href="'.$tagurl.'" target="_blank" class="label label-info">'.$interest->name.'</a></li>';
        }

        $taglist .= '</ul></div>';
        $popup = '<table border="0" cellspacing="0" cellpadding="0"><tr><td style="width:50px padding: 0 10px 0 0;vertical-align: initial">'.$OUTPUT->user_picture($participant, array('size' => 50, 'popup' => true)).'</td>'
            ...
            .'<a href="'.$profilelink.'" target="_blank" class="leaflet-viewprofil"><i class="fa fa-shield-alt" aria-hidden="true"></i> '.get_string('badgepopuplabel', 'local_interactive_map', $participant->badges).'</a></td></tr>'
