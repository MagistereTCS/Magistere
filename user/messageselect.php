...
// => QGO
$localmessage = optional_param('localmessage','',PARAM_BOOL);
$id_recipients = optional_param('rs','',PARAM_TEXT);

if($localmessage==1){
	$user_list = array();
	/*
	 $recipients_a = explode(',', $id_recipients);
	 
	 $_SESSION['recipients'] = $recipients_a;
	 */
	$url = new moodle_url('/local/mail/create.php', array('userid' => $USER->id, 'c' => $id, 'type' => 3, 'rs' => $id_recipients));
	$url = str_replace('&amp;', '&', $url);
	header('Location: '.$url);
}else{
