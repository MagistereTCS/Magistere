...
class participants_table extends \table_sql {
    ...
    public function col_fullname($data) {
        ...
        $out = $OUTPUT->user_picture($data, array('size' => 35, 'courseid' => $this->course->id, 'includefullname' => false));

        $link = new \moodle_url('/user/view.php', array('id' => $data->id , 'course' => $this->course->id));
        $a = \html_writer::tag('a', $data->firstname.' '.$data->lastname, array('href' => $link));

        $p = \html_writer::tag('p', $a);
