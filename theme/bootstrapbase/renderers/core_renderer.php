...
class theme_bootstrapbase_core_renderer extends core_renderer {
    ...
    public function navbar() {
        ...
        if(course_get_format($courseid) !== "topics" && isset($currentsection) && $currentsection == 0){

            $displayvalue = html_writer::link(new moodle_url('/course/view.php', array('id'=>$courseid , 'section' => 1)),get_section_name($courseid, 1));
            $list_items.="<li>$divider$displayvalue</li>";
        }
    ...
    protected function render_custom_menu_item(custom_menu_item $menunode, $level = 0 ) {
        ...
        if ($menunode->has_children()) {
            ...
            $content .= html_writer::start_tag('a', $linkParams);
            ...
            if (preg_match("/^#+$/", $menunode->get_text())) {
                ...
                $content = '<li>';
                if ($menunode->get_url() !== null) {
                    $url = $menunode->get_url()->out();
					$urlpagepath = $PAGE->url->out();

                    $urlpagepath = str_replace($CFG->wwwroot."/", "/",$urlpagepath);
                    $urllinkpath = str_replace($CFG->wwwroot."/", "/",$url);
                    $is_home = strpos($urlpagepath, '/my/index');

                    if(($urlpagepath == $urllinkpath) || ($is_home !== false && ($urllinkpath == '/my/' || $urllinkpath == '/my/index.php') )){
