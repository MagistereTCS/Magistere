...
YUI.add('moodle-core_filepicker', function(Y) {
    ...
    Y.Node.prototype.fp_display_filelist = function(options, fileslist, lazyloading) {
        ...
        /** return true if in centralized repository (different attributes in FileManager and FilePicker) */
        var file_get_iscentralized = function(node) {
            return (node.iscentralized == 'true'?'true':'false');
        };
        ...
        var initialize_table_view = function() {
            var cols;
            if ( file_get_iscentralized(fileslist[0]) == 'true' )
            {
            	cols = [
	                {key: "displayname", label: M.util.get_string('name', 'moodle'), allowHTML: true,
	                    sortable: true, sortFn: sortFoldersFirst},
	                {key: "type", label: M.str.repository.type, allowHTML: true,
		                sortable: true, sortFn: sortFoldersFirst},
	                {key: "domain", label: M.str.repository.domain, allowHTML: true, formatter: formatValue,
	                    sortable: true, sortFn: sortFoldersFirst},
	                {key: "creator", label: M.str.repository.creator, allowHTML: true,
		                sortable: true, sortFn: sortFoldersFirst},
                    {key: "lasteditdate", label: M.str.repository.lasteditdate, allowHTML: true, formatter: formatValue,
	                    sortable: true, sortFn: sortFoldersFirst}
	            ];
            }else{
            	cols = [
       ...
       classnamecallback : function(node) {
        ...
        view_as_list: function(appenditems) {
            ...
            if (!this.filelist || this.filelist.length==0 && (!this.filepath || !this.filepath.length)) {
                if (this.active_repo.iscentralized)
            	{
            		this.display_error(M.str.repository.nofilesavailable_centralized, 'nofilesavailable');
            	}else{
                    this.display_error(M.util.get_string('nofilesavailable', 'repository'), 'nofilesavailable');
                }
                return;
        ...
        select_file: function(args) {
            ...
            // TCS - MD - 22/04/2020 - #3874 - Added 2nd tarnary operator and "args.icon"
            var imgnode = Y.Node.create('<img/>').
                set('src', args.realthumbnail ? args.realthumbnail : (args.thumbnail ? args.thumbnail : args.icon)).
        ...
        setup_select_file: function() {
            ...
            getfile.on('click', function(e) {
                ...
                if (this.options.crexternallink || ((this.options.externallink || this.options.env != 'editor') &&
        ...
        parse_repository_options: function(data, appendtolist) {
            ...
            this.active_repo.iscentralized = (data.login || data.iscentralized);
            this.active_repo.noreload = (data.login || data.noreload);
        ...
        print_header: function() {
            ...
            if(!r.nosearch && !r.noreload || r.noreload && !window.isformloaded) {
                    ...
                    callback: function(id, obj, args) {
                        if (obj.repo_id == scope.active_repo.id && obj.form) {
                            ...
                            if (r.noreload)
                            {
                                ...
                                $(".datepicker_after").datepicker( "option", "monthNames", ["janvier", "f&eacute;vrier", "mars", "avril", "mai", "juin", "juillet", "ao&ucirc;t", "septembre", "octobre", "novembre", "d&eacute;cembre"] );
                                $(".datepicker_after").datepicker( "option", "monthNamesShort", ["janv.", "f&eacute;vr.", "mars", "avril", "mai", "juin", "juil.", "ao&ucirc;t", "sept.", "oct.", "nov.", "d&eacute;c."] );
                                $(".datepicker_after").datepicker( "option", "dayNames", ["dimanche", "lundi", "mardi", "mercredi", "jeudi", "vendredi", "samedi"] );
                                $(".datepicker_after").datepicker( "option", "dayNamesShort", ["dim.", "lun.", "mar.", "mer.", "jeu.", "ven.", "sam."] );
                                $(".datepicker_after").datepicker( "option", "dayNamesMin", ["D","L","M","M","J","V","S"] );
                                $(".datepicker_after").datepicker( "option", "dateFormat", "dd/mm/yy");
                                $(".datepicker_after").datepicker( "option", "weekHeader", "Sem.");
                                $(".datepicker_after").datepicker( "option", "firstDay", 1);
                                $(".datepicker_after").datepicker( "option", "currentText", "Aujourd\"hui");
                                $(".datepicker_after").datepicker( "option", "closeText", "Fermer");
                                $(".datepicker_after").datepicker( "option", "prevText", "Pr&eacute;c&eacute;dent");
                                $(".datepicker_after").datepicker( "option", "nextText", "Suivant");
                                $(".datepicker_after").datepicker( "option", "isRTL", false);
                                $(".datepicker_after").datepicker( "option", "showMonthAfterYear", false);

                              $(".datepicker_after").on("click", "", function(){
                                    $(".datepicker_after").datepicker();
                                    $(this).datepicker("show");
                                });

                              $(".datepicker_after_img").on("click", "", function(){
                                $(".datepicker_after").datepicker("show");
                                });
...
require(['jqueryui'], function(){
    $('head').append('<link rel="stylesheet" type="text/css" href="'+M.cfg.wwwroot+'/lib/jquery/ui-1.12.1/jquery-ui.min.css">');
