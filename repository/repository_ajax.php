...
switch ($action) {
    ...
    case 'download':
        ...
        if ((is_array($accepted_types) and in_array('*', $accepted_types)) or $accepted_types == '*') {
            ...
            // special case for indexation thumbnail and scorm
            if(in_array('.thumbnail', $accepted_types)){
                $accepted_types = array_merge($accepted_types, $CFG->centralizedresources_allow_filetype['image']);
            }

            if(in_array('.scorm', $accepted_types)){
                $accepted_types[] = '.zip';
            }

            // here the dot is required in order to moodle to recognize the type
            foreach($accepted_types as &$value){
        ...
        if ($allowexternallink and $linkexternal === 'yes' and ($repo->supported_returntypes() & FILE_EXTERNAL)) {
            ...
            $info['type'] = 'link';
            $info['url'] = $reference;
            // TCS - MD - 17/04/2020 - #3874 - START
            $info['link'] = $link;
            // TCS - MD - 17/04/2020 - #3874 - END
            ajax_check_captured_output();
