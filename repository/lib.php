...
abstract class repository implements cacheable_object {
    ...
    public static function get_instances($args = array()) {
        ...
        if ($args['type'] !== null) {
            if($args['type'] == 'nocentralizedresources'){
                $sql .= " AND r.type != :type";
                $params['type'] = 'centralizedresources';
            } else {
                $sql .= " AND r.type = :type";
...
function initialise_filepicker($args) {
    ...
    // TCS 2019 JBL : ajout pour plugin Atto CentralizedMedia
    $type = null;
    if (isset($args->type)) {
        $type = $args->type;
    }
    ...
    $repositories = repository::get_instances(array(
        ...
        'type'=>$type // TCS 2019 JBL : ajout pour plugin Atto CentralizedMedia
    ));
