...
class completion_info {
    ...
    public function display_help_icon() {
        global $PAGE, $DB, $USER, $OUTPUT, $COURSE;
        $result = '';
        if ($this->is_enabled() && !$PAGE->user_is_editing() && isloggedin() && !isguestuser()) {

            // Added by TCS for #3696, updated in #4016, code mainly from reverted commit id : 7a29b7d355917f415199f8eae580b5dc3008fd65
            // "dynamic" variabled added to support dynamic concatenation of string inside popup
            $dynamic = [];

            $sectionNumber = optional_param('section', null, PARAM_INT);
            $cminfo = get_fast_modinfo($COURSE->id);

            $modules = [];
            if ($sectionNumber) {
                // if there is a section, only take its modules
                $sections = $cminfo->get_sections();
                if (isset($sections[$sectionNumber])) {
                    $modulesId = $sections[$sectionNumber];
                    foreach($modulesId as $modId) {
                        $modules[] = $cminfo->get_cm($modId);
                    }
                }
            } else {
                $modules = $cminfo->get_cms();
            }


            $completionEnabled = false;
            foreach ($modules as $activity){
                if ($activity->uservisible && $activity->modname == "completionmarker" || $activity->completion > 0) {
            ...
            if ($completionEnabled) {
                ...
                // activité avec achèvement
                $done = [];
                foreach ($modules as $activity){
                    if($activity->uservisible && $activity->completion > 0 && $activity->modname != "completionmarker") {
                        $done[] = $activity->name;
                    }
                }

                if($done){
                    $dynamic[] = "aa";// aa = activité avec achèvement
                    $a = ' '.implode(', ',$done);
                }

                // marqueur d'achèvement
                foreach ($modules as $activity){
                    if($activity->uservisible && $activity->modname == "completionmarker") {
                        $dynamic[] = "ma";// ma = marqueur d'achèvement = completionmarker;
                        break;
                    }
                }
                ...
                foreach ($PAGE->blocks->get_regions() as $region){
                    ...
                    foreach ($blocks as $block){
                        if($block->name() === "completion_progress") {
                            $block_instance = block_instance('completion_progress', $block->instance);

                            if ($block_instance->config && $block_instance->config->activitiesincluded =='selectedactivities') {
                                $selectactivites = $block_instance->config->selectactivities;
                                // search the included mod instances
                                foreach ($modules as $activity){
                                    if ($activity->completion > 0 && $activity->uservisible) {
                                        $searchStr = $activity->modname.'-'.$activity->instance;
                                        if (in_array($searchStr, $selectactivites)) {
                                            $dynamic[] = "bds";// bds = barre de suivi = block_completion_progress
                                            break 2;
                                        }
                                    }
                                }
                ...
                $result .= html_writer::tag('div', get_string('yourprogress','completion') .
                    $OUTPUT->info_icon('completionicons', 'completion', '', $dynamic, $a), array('id' => 'completionprogressid',
