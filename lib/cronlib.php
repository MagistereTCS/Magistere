...
function cron_run() {
    ...
    while (!\core\task\manager::static_caches_cleared_since($timenow) &&
        ...
        try{
            ...
            mtrace('Exception catched : '.print_r($e,true));
        }
        unset($task);
    }
    $CFG->disable_adhoc_tasks = array();
    $CFG->enable_adhoc_task = false;
    if (!isset($CFG->enable_adhoc_task) || (isset($CFG->enable_adhoc_task) && $CFG->enable_adhoc_task)){
        // Run all adhoc tasks.
        while (!\core\task\manager::static_caches_cleared_since($timenow) &&
               $task = \core\task\manager::get_next_adhoc_task($timenow)) {
            if (!isset($CFG->disable_adhoc_tasks) || (is_array($CFG->disable_adhoc_tasks) && !in_array(get_class($task),$CFG->disable_adhoc_tasks))){
...
function cron_run_inner_scheduled_task(\core\task\task_base $task) {
    ...
    try {
        ...
        $event = \local_taskmonitor\event\task_scheduled_completed::create(
            array(
                'contextid'=>context_system::instance()->id,
                'other' => array(
                    'classname'=>get_class($task),
                    'query'=>($DB->perf_get_queries() - $predbqueries),
                    'starttime'=>floor($pretime),
                    'runtime'=>floor((microtime(1) - $pretime)*1000000),
...
function cron_run_inner_adhoc_task(\core\task\adhoc_task $task) {
    ...
    try {
        ...
        $event = \local_taskmonitor\event\task_adhoc_completed::create(
            array(
                'contextid'=>context_system::instance()->id,
                'other' => array(
                    'classname'=>get_class($task),
