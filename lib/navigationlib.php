...
class global_navigation extends navigation_node {
    ...
    protected function show_my_categories() {
        ...
        if ($this->showmycategories === null) {
            ...
            $isformateur = user_has_role_assignment($USER->id,$role_formateur->id);
            $this->showmycategories = (!empty($CFG->navshowmycoursecategories) || $isformateur) && coursecat::count_all() > 1;
        }
    ...
    public function add_course(stdClass $course, $forcegeneric = false, $coursetype = self::COURSE_OTHER) {
        ...
        if ($issite) {
            ...
            $role_formateur = $DB->get_record('role', array('shortname' => "formateur"));
            $isformateur = user_has_role_assignment($USER->id,$role_formateur->id);
            if ((!empty($CFG->navshowmycoursecategories) || $isformateur) && ($parent = $this->rootnodes['mycourses']->find($course->category, self::TYPE_MY_CATEGORY))) {
...
class settings_navigation extends navigation_node {
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if ($this->page->user_allowed_editing()) {
            $centralizedurl = new moodle_url("/local/centralizedresources/view.php?", array('controller'=>'manageresource','action'=>'default','courseid'=>$course->id));
            $coursenode->add(get_string('centralizedresources'), $centralizedurl, self::TYPE_SETTING, null, 'centralizedresources', new pix_icon('i/settings', ''));
