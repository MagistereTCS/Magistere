...
function get_courses_search($searchterms, $sort, $page, $recordsperpage, &$totalcount,
    ...
    if ($DB->get_dbfamily() == 'oracle') {
        ...
        $concat = $DB->sql_concat("COALESCE(c.summary, '')", "' '", 'c.fullname', "' '", 'c.idnumber', "' '", 'c.shortname', "' '", 'IFNULL(lgsc.dispositif_id, "")', "' '", 'IFNULL(lgsc.module_id, "")'); // Mantis 1270 JB : 20/03/2019
    }
    ...
    $ccselect = ', ' . context_helper::get_preload_record_columns_sql('ctx');
    $ccjoin = "LEFT JOIN {context} ctx ON (ctx.instanceid = c.id AND ctx.contextlevel = :contextlevel)";
    $ccjoin .= " LEFT JOIN {local_gaia_session_course} lgsc ON (lgsc.course_id = c.id)"; // Mantis 1270 JB : 20/03/2019
    $params['contextlevel'] = CONTEXT_COURSE;
...
/// Store course lastaccess times for the current user
    ...
    /// Store course dayaccess times for the current user
    $timeday = mktime(12,0,0);
    $user_dayaccess = $DB->record_exists('user_dayaccess', array('userid'=>$USER->id, 'courseid'=>$courseid, 'timeaccess'=>$timeday));

    if ($user_dayaccess === false) {
        // Insert new dayaccess for the course
        $last = new stdClass();
        $last->userid     = $USER->id;
        $last->courseid   = $courseid;
        $last->timeaccess = $timeday;
        try {
            $DB->insert_record_raw('user_dayaccess', $last, false);
        } catch (dml_write_exception $e) {
            // During a race condition we can fail to find the data, then it appears.
            // If we still can't find it, rethrow the exception.
            $dayaccess = $DB->get_field('user_dayaccess', 'timeaccess', array('userid' => $USER->id,
                                                                                'courseid' => $courseid));
            if ($dayaccess === false) {
                throw $e;
            }
