...
class user_picture implements renderable {
    ...
    public function get_url(moodle_page $page, renderer_base $renderer = null) {
        ...
        if ($this->user->picture > 0) {
            ...
            $avatar = $dbconn->get_record('cr_avatars', array('id' => $this->user->picture));

            // On verifie si une valeur existe dans la table avatar
            if($avatar !== false){
                ...
                $url = local_magisterelib_profile_picture::create($this->user);
            	// $url = moodle_url::make_pluginfile_url($contextid, 'user', 'icon', NULL, $path, $filename);
            	// $url->param('rev', $this->user->picture);
            }
        ...
        return local_magisterelib_profile_picture::create($this->user);
    }
}
 ...
 */
class info_icon implements renderable, templatable {

    /**
     * @var string lang pack identifier (without the "_info" suffix),
     * both get_string($identifier, $component) and get_string($identifier.'_info', $component)
     * must exist.
     */
    public $identifier;
    ...
    /**
     * @var array of strings to support concatenation of multiple strings
     */
    public $dynamic = [];

    /**
     * @var string variable string to be added
     */
    public $a = '';
     ...
     */
    public function __construct($identifier, $component, $dynamic, $a = '') {
    ...
    public function diag_strings() {
        ...
        if (!$sm->string_exists($this->identifier, $this->component)) {
            debugging("Info title string does not exist: [$this->identifier, $this->component]");
        }
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data->alt = $alt;
        $data->icon = (new pix_icon('docs', $alt, 'core', ['class' => 'icondocs']))->export_for_template($output);
