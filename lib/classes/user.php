...
class core_user {
    ...
    public static function update_picture(stdClass $usernew, $filemanageroptions = array()) {
        ...
        $user = core_user::get_user($usernew->id, '*', MUST_EXIST);
        ...
        if (!empty($usernew->deletepicture)) {
            ...
            if (($iconfiles = $fs->get_area_files($context->id, 'user', 'newicon')) && count($iconfiles) == 2) {
                ...
                if ($iconfile = $file->copy_content_to_temp()) {
                    ...
                    $icon_file = $fs->get_file_by_id($newpicture);
                    
                    $path = $CFG->centralized_avatar_path;
                    if(!empty($path)){
                        // Verification que le dossier avatar existe
                        if (!file_exists($path)) {
                            // On le cree si c'est pas le cas
                        ...
                        $folder = $path . '/' . substr($hashname, 0, 2) . '/';
                        ...
                        $filepath = $folder . $hashname .'.png'; // Path du fichier une fois deplace
                        
                        // On copie le fichier vers sa nouvelle destination
                        $icon_file->copy_content_to($filepath);
                        
                        // Suppression du fichier dans le dossier d'origine
                        $file->delete();
                        $icon_file->delete();
                        ...
                        if($user->picture != 0){
                            ...
                            //cas special : certains utilisateurs n'ont pas d'avatar alors que l'id existe
                            if($avatar !== false){
                                $avatar_folder = $path . '/' . substr($avatar->hashname, 0, 2) . '/';
                                
                                $pathname = $avatar_folder . $avatar->hashname .'.png'; // Path du fichier une fois deplace
                                if(file_exists($pathname)){
                                    unlink($pathname);
                                }
                                $avatar->hashname = $hashname;
                                $avatar->email = $user->email;
                                // Id de l'entree presente en base dans la table cr_avatars
                                $dbconn->update_record('cr_avatars', $avatar);
