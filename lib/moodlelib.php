 ...
 */
define('HUB_HUBDIRECTORYURL', "http://localhost");
 ...
 */
define('HUB_MOODLEORGHUBURL', $CFG->hubserver_url);
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if (!isloggedin()) {
    	...
    	if (strpos(qualified_me(),'/login') === false)
    	{
    ...
    if ((!isloggedin() or isguestuser()) && !empty($SESSION->has_timed_out) && !empty($CFG->dbsessions)) {
    	...
    	if (!isloggedin() && (!empty($_SERVER['HTTP_SHIB_APPLICATION_ID']) || !empty($_SERVER['Shib-Application-ID']) || preg_match('/_shibsession_/i', $_SERVER['HTTP_COOKIE']))){
...
function authenticate_user_login($username, $password, $ignorelockout=false, &$failurereason=null, $logintoken=false) {
    ...
    foreach ($auths as $auth) {
        ...
        //TCS BEGIN 2017/07/26 - VSE (old NNE)
        if($authplugin->authtype == 'shibboleth' && !isfrontal()){
        	//here username contains the user's mail
        ...
        //TCS END 2017/07/26 - VSE (old NNE)
...
function get_complete_user_data($field, $value, $mnethostid = null) {
    ...
    if (! $user = $DB->get_record_select('user', $constraints, $params)) {
    	...
    	if (!isfrontal() && $user->auth == 'shibboleth')
    	{
	    	//Load data from the frontal
	    	require_once($CFG->dirroot.'/local/magisterelib/userProfileSynchronisation.php');
	    	$userProfileSynchronisation = new UserProfileSynchronisation();
	    	$user = $userProfileSynchronisation->updateLocalUser($user);
	    }
    }
    //END VSE (old NNE)
...
// TCS - ALTI - URL SECURE - QGO - VSE

//function secure_url($url_base, $secure_link_expires){
//	global $CFG;
//	$secure_link_expires += time();
//	// $uri of the file
//	$uri = str_replace($CFG->centralizedFolder, '/', $url_base);
//	//hash
//	$hash = md5($CFG->secure_link_salt . $uri . $secure_link_expires, true);
//	$hash = rtrim(strtr(base64_encode($hash), '+/', '-_'), '=');
//	$secure_url = $CFG->centralizedFolder.$hash.'/'.$secure_link_expires.$uri;
//	return $secure_url;
//}



//TCS BEGIN 2015/03/30 - NNE - 377
function get_resource_centralized_secure_url($url_base, $filehash, $secure_link_expires, $is_diapo = false, $force_download = false, $updatelastuser = true){
    $secure_url = secure_url($url_base, $filehash, $secure_link_expires, $is_diapo, $force_download);

    $cdb = get_centralized_db_connection();

    $hashname = substr($filehash, 0, 40);
    $createdate = substr($filehash, -10);

    $get_resource_from_db_sql = 'SELECT id FROM cr_resources WHERE hashname="' . $hashname . '" AND createdate="' . $createdate.'"';

    $resource = $cdb->record_exists_sql($get_resource_from_db_sql);

    if($resource && $updatelastuser){
        $update_lastusedate_sql = 'UPDATE cr_resources SET lastusedate=' . time() . ', views=views+1 WHERE hashname="' . $hashname . '" AND createdate="' . $createdate.'"';

        if($cdb->execute($update_lastusedate_sql) == false){
            return false;
        }
    }
...
//TCS END 2015/03/30 - NNE - 377

//TCS BEGIN 2019/08/29 - NNE - 3162
function get_resource_centralized_secure_preview_url($url_base, $filehash, $secure_link_expires, $is_diapo = false){
    $secure_url = secure_url($url_base, $filehash, $secure_link_expires, $is_diapo, false);
    return $secure_url;
}
//TCS END 2019/08/29 - NNE - 3162



// TCS BEGIN - VSE - 483
function secure_url($url_base, $filehash, $secure_link_expires, $is_diapo = false, $force_download = false){
    global $CFG;
    $secure_link_expires += time();
    // $uri of the file
    $uri = str_replace($CFG->centralizedFolder, '/', $url_base);
    ...
    if ($is_diapo)
    {
        $hash = md5($CFG->secure_link_salt . '/' . $filehash . '/' . $secure_link_expires, true);
        $hash = rtrim(strtr(base64_encode($hash), '+/', '-_'), '=');
        $secure_url = $CFG->centralizedFolder.'d/'.$hash.'/'.$secure_link_expires.'/'.$filehash.$uri;
    }else{
        $hash = md5($CFG->secure_link_salt . '/' . $filehash . $uri . $secure_link_expires, true);
        $hash = rtrim(strtr(base64_encode($hash), '+/', '-_'), '=');
        $force = ($force_download?'dl/':'');
        $secure_url = $CFG->centralizedFolder.$force.$hash.'/'.$secure_link_expires.'/'.$filehash.$uri;
    }
    return $secure_url;
}
// TCS END - VSE - 483
...
// TCS BEGIN - VSE - 585
function add_to_progress_activity($course_id, $course_module_id, $user_id, $status, $update_on_insert_fail = false)
{
    global $USER, $DB;
    ...
    try
    {
        ...
        $sql = 'INSERT INTO mdl_progress_activities (course_id, course_module_id, user_id, add_date, module_name, status) 
      SELECT '.$course_id.', '.$course_module_id.', '.$user_id.', '.time().', mo.name, "'.$status.'" FROM mdl_modules mo INNER JOIN mdl_course_modules cm ON mo.id = cm.module WHERE cm.id = '.$course_module_id.'';

        if ( $update_on_insert_fail )
        {
        ...
        //debugging('Error: Could not insert a new entry in the progress block table. '. $e->error, DEBUG_ALL);
    }

}
// TCS END - VSE - 585

//TCS BEGIN 2015/03/30 - VSE -NNE - 377
function get_centralized_db_connection()
{
    global $CFG, $CENTRALIZED_DB;
    // singleton
    if (!isset($CENTRALIZED_DB)) {

        if (!$CENTRALIZED_DB = moodle_database::get_driver_instance($CFG->dbtype, $CFG->dblibrary)) {
            throw new dml_exception('dbdriverproblem', "Unknown driver $CFG->dblibrary/$CFG->dbtype");
        }

        try {
            $CENTRALIZED_DB->connect($CFG->centralized_dbhost, $CFG->centralized_dbuser, $CFG->centralized_dbpass, $CFG->centralized_dbname, '', $CFG->dboptions);
        } catch (moodle_exception $e) {
...
//TCS END 2015/03/30 - VSE - NNE - 377



//TCS BEGIN 2015/08/17 - NNE
function update_user_profile_from_frontal($username)
{
    global $CFG, $DB;
    if(isfrontal()){return true;}
    require_once($CFG->libdir.'/filelib.php');

    //check if exists on frontal
    $curl = new curl;
    ...
    $resp = $curl->post($CFG->frontal_url_ws, array(
        'fparams' => $data,
        'fname' => 'get',
...
//	echo '###'.print_r($resp,true).'###'; die();
    $resp = (array)clean_decode_json($resp);
//      echo '###'.print_r($resp,true).'###'; die();
    if(isset($resp['id'])){
        $userUpdate = $DB->get_record('user', array('username' => $username));

        foreach($CFG->ws_user_profile_allowed_fields as $field){
            if(isset($resp[$field])){
                if($resp[$field] != NULL){
                    $userUpdate->{$field} = $resp[$field];
                }
            }
        }
        ...
        if (isset($resp['mainacademy']) && $resp['mainacademy'] != '')
        {
            user_set_mainacademy($resp['mainacademy']);
        }
    }
}

function update_user_profile_to_frontal($userid)
{
    global $CFG, $DB;
    if(isfrontal()){return true;}
    ...
    $dbdata = $DB->get_record('user', array('id' => $userid));
    ...
    if($dbdata !== false){
        ...
        $resp = clean_decode_json($resp);

        return $resp->has_been_update;
    }
}

function clean_decode_json($json)
{
    $cjson = stripslashes($json);
    $cjson = substr($cjson, 1, -1);

    return json_decode($cjson);
}
//TCS END 2015/08/17 - NNE
...
/**
 * Open memcached server connection, set the global variable and return it
 * @return Memcache instance
 */
function mmcached_connect()
{
    global $MMCACHED, $CFG;
    if (!isset($MMCACHED) || $MMCACHED == null || !is_object($MMCACHED))
    {
        $MMCACHED = new Memcached();

        $MMCACHED->addServer($CFG->memcached_host, $CFG->memcached_port);
    }
 ...
 * @param int $expire The expiration delay in seconds.
 * @return Returns TRUE on success or FALSE on failure.
 */
function mmcached_add($key,$value,$expire = null)
{
    global $CFG;
    if ($expire == null)
    {
        $expire = $CFG->memcached_expire;
    }
    return mmcached_connect()->add($key, $value, $expire);
}
 ...
 * @return Returns TRUE on success or FALSE on failure.
 */
function mmcached_set($key,$value,$expire = null)
{
    global $CFG;
    if ($expire == null)
    {
    ...
    return mmcached_connect()->set($key, $value, $expire);
}
 ...
 * @return mixed Returns the string associated with the key or an array of found key-value pairs when key is an array. Returns FALSE on failure, key is not found or key is an empty array.
 */
function mmcached_get($key)
{
    return mmcached_connect()->get($key);
}
 ...
 */
function mmcached_delete($key)
{
    return mmcached_connect()->delete($key);
}

/**
 *
 * @return String return the user memcached unique identifier
 */
function get_mmid()
{
    if (!isset($_COOKIE['mmid']))
    {
//        echo '##COOKIE mmid not found##';
//        echo '###'.print_r($_COOKIE,true).'###';
        $mmid = generateRandomString(20);
        while (mmcached_get('mmid_session_'.$mmid) !== false)
        {
            $mmid = generateRandomString(30);
        }
        setcookie('mmid',$mmid,0,'/',$_SERVER['HTTP_HOST'],true);
        $_COOKIE['mmid'] = $mmid;
        mmcached_add('mmid_session_'.$mmid, $mmid);
    }
    return $_COOKIE['mmid'];
}
 ...
 * @param int $userid User id, use current userid if not set
 * @return bool Returns TRUE on success or FALSE on failure.
 */
function user_set_mainacademy($academy, $userid = null)
{
    global $DB, $USER;
    if ( $userid == null){ $userid = $USER->id;}
    if (isset($userid) && $userid > 0)
    {
        return $DB->execute("INSERT INTO {user_info_data} (`userid`,`fieldid`,`data`,`dataformat`) VALUES('".$userid."',(SELECT id FROM {user_info_field} WHERE shortname = 'mainacademy'),'".$academy."','0') ON DUPLICATE KEY UPDATE data=VALUES(data)");
    }
    return false;
}
...
/**
 * @param int $userid User id, use current userid if not set
 * @return mainacademy field data or false if not found
 */
function user_get_mainacademy($userid = null)
{
    global $DB, $USER;
    if ( $userid == null){ $userid = $USER->id;}
    if (isset($userid) && $userid > 0)
    {
        $field = $DB->get_record_sql("SELECT data FROM {user_info_data} WHERE userid = '".$userid."' AND fieldid = (SELECT id FROM {user_info_field} WHERE shortname = 'mainacademy')");
...
function user_set_frontal_mainacademy($academy,$userid=null)
{
    global $USER, $DB, $CFG;
    require_once($CFG->libdir.'/filelib.php');
    ...
    if ($userid != null && $userid != $USER->id)
    {
        $luser = $DB->get_record_sql('SELECT username FROM {user} WHERE auth = "shibboleth" AND id = "' . $userid . '"', null, IGNORE_MULTIPLE);
        $username = $luser->username;
    }
    
    if(databaseConnection::instance()->get('frontal') === false){
        return false;
    }
    
    $user = databaseConnection::instance()->get('frontal')->get_record_sql('SELECT * FROM {user} WHERE auth = "shibboleth" AND username = "' . $username . '"', null, IGNORE_MULTIPLE);
    
    if($user !== false){
        $hasBeenUpdate = databaseConnection::instance()->get('frontal')->execute("UPDATE {user_info_data} SET data = '".$academy."' WHERE fieldid = (SELECT id FROM {user_info_field} WHERE shortname='mainacademy') AND userid = (SELECT id FROM {user} WHERE username = '".$username."')");
    }
...
function user_get_frontal_mainacademy($username=null)
{
    global $USER,$CFG;
    ...
    $user = databaseConnection::instance()->get('frontal')->get_record_sql('SELECT data FROM {user_info_data} WHERE fieldid = (SELECT id FROM {user_info_field} WHERE shortname="mainacademy") AND userid = (SELECT id FROM {user} WHERE auth = "shibboleth" AND username = "'.$username.'")', null, IGNORE_MULTIPLE);
    
    if($user !== false){
        if (array_key_exists($user->data,get_magistere_academy_config())) {
            return $user->data;
        }
    }
